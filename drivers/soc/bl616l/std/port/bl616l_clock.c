...
uint32_t bflb_clk_get_peripheral_clock(uint8_t type, uint8_t idx)
{
    if (type == BFLB_DEVICE_TYPE_ADC) {
        return Clock_Peripheral_Clock_Get(BL_PERIPHERAL_CLOCK_GPADC);
    } else if (type == BFLB_DEVICE_TYPE_AUDIOADC) {
        return Clock_Peripheral_Clock_Get(BL_PERIPHERAL_CLOCK_AUADC);
    } else if (type == BFLB_DEVICE_TYPE_AUDIODAC) {
        return Clock_Peripheral_Clock_Get(BL_PERIPHERAL_CLOCK_AUDAC);
        ...
        return Clock_Peripheral_Clock_Get(BL_PERIPHERAL_CLOCK_PWM);
