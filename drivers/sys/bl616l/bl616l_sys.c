...
void bl_cpu_sysmap_init(bool dcache_preload_en, bool dcache_amr_en)
{
    ...
    /* 1. 0x00000000~0x60FC0000: Strong-Order, Non-Cacheable, Non-Bufferable */
    putreg32(0x60FC0000 >> SYSMAP_BASE_OFFSET, (sysmap_base + 0x00));
    putreg32(SYSMAP_ATTR_STRONG_ORDER, (sysmap_base + 0x04));
    sysmap_base += 0x08;

    /* 2. ocram 0x60FC0000~0x60FF8000: Weak-Order, Cacheable, Bufferable */
    putreg32(0x60FF8000 >> SYSMAP_BASE_OFFSET, (sysmap_base + 0x00));
    ...
    putreg32((0x61020000 - em_size) >> SYSMAP_BASE_OFFSET, (sysmap_base + 0x00));
    putreg32(SYSMAP_ATTR_CACHE_ABLE | SYSMAP_ATTR_BUFFER_ABLE, (sysmap_base + 0x04));
    sysmap_base += 0x08;

    /* 4. empty 0x61020000~0x80000000: Strong-Order, Non-Cacheable, Non-Bufferable */
    putreg32(0x80000000 >> SYSMAP_BASE_OFFSET, (sysmap_base + 0x00));
    putreg32(SYSMAP_ATTR_STRONG_ORDER, (sysmap_base + 0x04));
    sysmap_base += 0x08;

    /* 5. flash(32M) 0x80000000~0x82000000: Weak-Order, Cacheable, Non-Bufferable */
    putreg32(0x82000000 >> SYSMAP_BASE_OFFSET, (sysmap_base + 0x00));
    putreg32(SYSMAP_ATTR_CACHE_ABLE, (sysmap_base + 0x04));
    sysmap_base += 0x08;

    /* 6. empty 0x82000000~0x88000000: Strong-Order, Non-Cacheable, Non-Bufferable */
    putreg32(0x88000000 >> SYSMAP_BASE_OFFSET, (sysmap_base + 0x00));
    putreg32(SYSMAP_ATTR_STRONG_ORDER, (sysmap_base + 0x04));
    sysmap_base += 0x08;

    /* 7. psram(16M) 0x88000000~0x89000000: Weak-Order, Cacheable, Bufferable */
    if (BL616L_PSRAM_INIT_DONE) {
        putreg32(0x89000000 >> SYSMAP_BASE_OFFSET, (sysmap_base + 0x00));
        putreg32(SYSMAP_ATTR_CACHE_ABLE | SYSMAP_ATTR_BUFFER_ABLE, (sysmap_base + 0x04));
        sysmap_base += 0x08;
    }

    /* 8. rom/others: Strong-Order, Non-Cacheable, Non-Bufferable */
