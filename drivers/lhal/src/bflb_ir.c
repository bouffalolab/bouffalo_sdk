...
void bflb_ir_swm_send(struct bflb_device_s *dev, uint16_t *data, uint32_t length)
{
    ...
    regval |= (min_data - 1) << IR_CR_IRTX_PW_UNIT_SHIFT;
    putreg32(regval, reg_base + IRTX_PULSE_WIDTH_OFFSET);
    ...
    for (i = 0; i < count; i++) {
        ...
        if ((i < count - 1) || (length % 2 == 0)) {
            ...
            /* Deal with pulse width data remained which is less than 2 */
...
int bflb_ir_feature_control(struct bflb_device_s *dev, int cmd, size_t arg)
{
    ...
    switch (cmd) {
        ...
        case IR_CMD_SWM_SET_UNIT_DATA:
            regval = getreg32(dev->reg_base + IRTX_PULSE_WIDTH_OFFSET);
            regval &= ~IR_CR_IRTX_PW_UNIT_MASK;
            regval |= (arg << IR_CR_IRTX_PW_UNIT_SHIFT) & IR_CR_IRTX_PW_UNIT_MASK;
            putreg32(regval, dev->reg_base + IRTX_PULSE_WIDTH_OFFSET);
            break;
