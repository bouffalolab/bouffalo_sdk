...
#define BFLB_GLB_BASE                 GLB_BASE
#define BFLB_USB_IRQ_NUM              BL702_IRQ_USB
...
#else
#error "No support USB_V1 for this chip"
#endif
...
{
    uint32_t fifo_addr = BFLB_USB_BASE + USB_EP0_TX_FIFO_WDATA_OFFSET + ep_idx * 0x10;
...
{
    uint32_t fifo_addr = BFLB_USB_BASE + USB_EP0_RX_FIFO_RDATA_OFFSET + ep_idx * 0x10;
...
static void bflb_usb_xcvr_config(void)
{
    ...
    regval = getreg32(BFLB_GLB_BASE + GLB_USB_XCVR_OFFSET);
    regval |= GLB_PU_USB;
    putreg32(regval, BFLB_GLB_BASE + GLB_USB_XCVR_OFFSET);
    ...
    putreg32(regval, BFLB_GLB_BASE + GLB_USB_XCVR_CONFIG_OFFSET);
...
static void bflb_usb_ep0_size(uint8_t ep_mps)
{
    ...
    regval = getreg32(BFLB_USB_BASE + USB_CONFIG_OFFSET);
    regval &= ~USB_CR_USB_EP0_SW_SIZE_MASK;
    regval |= (ep_mps << USB_CR_USB_EP0_SW_SIZE_SHIFT);
    putreg32(regval, BFLB_USB_BASE + USB_CONFIG_OFFSET);
}
...
static void bflb_usb_ep_config(uint8_t ep_idx, uint8_t ep_type, uint8_t dir, uint16_t ep_mps)
{
    ...
    regval = getreg32(BFLB_USB_BASE + USB_EP1_CONFIG_OFFSET + 4 * (ep_idx - 1));
    ...
    putreg32(regval, BFLB_USB_BASE + USB_EP1_CONFIG_OFFSET + 4 * (ep_idx - 1));
}
...
{
    return (getreg32(BFLB_USB_BASE + USB_EP0_FIFO_STATUS_OFFSET + 0x10 * ep_idx) & USB_EP0_RX_FIFO_CNT_MASK) >> USB_EP0_RX_FIFO_CNT_SHIFT;
}

// static uint16_t bflb_usb_get_txcount(uint8_t ep_idx)
// {
//     return (getreg32(BFLB_USB_BASE + USB_EP0_FIFO_STATUS_OFFSET + 0x10 * ep_idx) & USB_EP0_TX_FIFO_CNT_MASK) >> USB_EP0_TX_FIFO_CNT_SHIFT;
// }
...
static bool bflb_usb_ep_isbusy(uint8_t ep_idx)
{
    ...
    if (ep_idx == 0) {
        regval = (getreg32(BFLB_USB_BASE + USB_CONFIG_OFFSET) & USB_STS_USB_EP0_SW_RDY);
    } else {
        regval = (getreg32(BFLB_USB_BASE + USB_EP1_CONFIG_OFFSET + 4 * (ep_idx - 1)) & USB_STS_EP1_RDY);
    }
...
static uint32_t bflb_usb_get_intstatus(void)
{
    ...
    regval = getreg32(BFLB_USB_BASE + USB_INT_STS_OFFSET);
    regval &= ~getreg32(BFLB_USB_BASE + USB_INT_MASK_OFFSET);
    regval &= getreg32(BFLB_USB_BASE + USB_INT_EN_OFFSET);
    return regval;
}
...
{
    uint32_t regval;
    regval = getreg32(BFLB_USB_BASE + USB_INT_CLEAR_OFFSET);
    regval |= int_clear;
    putreg32(regval, BFLB_USB_BASE + USB_INT_CLEAR_OFFSET);
}
...
int usb_dc_init(uint8_t busid)
{
    ...
    putreg32(regval, BFLB_USB_BASE + USB_INT_EN_OFFSET);
    ...
    putreg32(regval, BFLB_USB_BASE + USB_INT_MASK_OFFSET);

    putreg32(0xffffffff, BFLB_USB_BASE + USB_INT_CLEAR_OFFSET);
...
int usbd_ep_open(uint8_t busid, const struct usb_endpoint_descriptor *ep)
{
    ...
    if (ep_idx != 0) {
        ...
        bflb_usb_ep_config(ep_idx, ep_type, dir, USB_GET_MAXPACKETSIZE(ep->wMaxPacketSize));
...
void USBD_IRQHandler(uint8_t busid)
{
        ...
        if (intstatus & (1 << (9 + 2 * ep_idx))) {
            if (g_bl_udc.in_ep[ep_idx].ep_enable) {
                ...
                if ((rx_count < g_bl_udc.out_ep[ep_idx].ep_mps) ||
                    ...
                    usbd_event_ep_out_complete_handler(busid, ep_idx, g_bl_udc.out_ep[ep_idx].actual_xfer_len);
                } else {
                    bflb_usb_ep_set_ready(ep_idx);
                }
            }
    ...
    if (intstatus & USB_EP0_SETUP_DONE_INT) {
        ...
        usbd_event_ep0_setup_complete_handler(busid, (uint8_t *)&g_bl_udc.setup);
    }
    if (intstatus & USB_EP0_IN_DONE_INT) {
        ...
        usbd_event_ep_in_complete_handler(busid, 0 | 0x80, g_bl_udc.in_ep[0].actual_xfer_len);
    ...
    if (intstatus & USB_EP0_OUT_DONE_INT) {
        ...
        usbd_event_ep_out_complete_handler(busid, 0, g_bl_udc.out_ep[0].actual_xfer_len);
