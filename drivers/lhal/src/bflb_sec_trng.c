...
int bflb_group0_request_trng_access(struct bflb_device_s *dev)
{
    ...
    return romapi_bflb_group0_request_trng_access(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
{
#ifdef romapi_bflb_group1_request_trng_access
    return romapi_bflb_group1_request_trng_access(dev);
#else
    uint32_t regval;
    uint32_t reg_base;
...
{
#ifdef romapi_bflb_group1_release_trng_access
    romapi_bflb_group1_release_trng_access(dev);
#else
    uint32_t reg_base;
