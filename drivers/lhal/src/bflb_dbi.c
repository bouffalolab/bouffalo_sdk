...
static void bflb_dbi_format_set(struct bflb_device_s *dev, uint8_t format)
{
    ...
    switch (format) {
        ...
        case DBI_PIXEL_INPUT_FORMAT_YUVN:
        case DBI_PIXEL_INPUT_FORMAT_VUY444:
        case DBI_PIXEL_INPUT_FORMAT_YUV444:
        ...
        case DBI_PIXEL_INPUT_FORMAT_VYUY:
            regval |= (DBI_PIXEL_INPUT_FORMAT_BGR_565 << DBI_FIFO_FORMAT_SHIFT);
            regval |= DBI_FIFO_YUV_MODE;
...
__UNUSED static void bflb_dbi_fill_fifo(struct bflb_device_s *dev, uint32_t words_cnt, uint32_t *data_buff)
{
    uint32_t reg_base;
    uint32_t regval;
    uint32_t fifo_cnt;
...
__UNUSED static uint32_t bflb_dbi_get_words_cnt_form_pixel(struct bflb_device_s *dev, uint32_t pixle_cnt)
{
    uint32_t reg_base;
    uint32_t regval;
    uint32_t words_cnt;
...
int bflb_dbi_send_cmd_data(struct bflb_device_s *dev, uint8_t cmd, uint8_t data_len, uint8_t *data_buff)
{
    ...
    if (data_len) {
        if (data_len <= DBI_FIFO_NUM_MAX * 4) {
            ...
            bflb_dbi_fill_fifo(dev, DBI_FIFO_NUM_MAX, (uint32_t *)data_buff);
