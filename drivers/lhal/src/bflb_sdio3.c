...
int bflb_sdio3_upld_push(struct bflb_device_s *dev, bflb_sdio3_trans_desc_t *trans_desc)
{
    ...
    *upld_desc = *trans_desc;
    ...
    bflb_sdio3_custom_reg_read(dev, SDIO3_CUSTOM_REG_FUNC_OFFSET(upld_desc->func) + SDIO3_CUSTOM_REG_FUNC_BLOCK_SIZE, &blk_size, 2);
...
static void bflb_sdio3_isr_cb_vol_switch(struct bflb_device_s *dev, uint32_t int_sta1)
{
    ...
    if (int_sta1 & SDIO3_CMD11_CLK_STOP) {
        ...
        /* callback */
        if (sdio3_irq_event_cb != NULL) {
            sdio3_irq_event_cb(sdio3_irq_arg, SDIO3_IRQ_EVENT_SIG1V8_SWITCH, NULL);
        }
        return;
    }
...
static void bflb_sdio3_isr(int irq, void *arg)
{
    ...
    /* 4. unknow int, callback */
    if (sdio3_irq_event_cb != NULL) {
        SDIO3_DRV_DBG("sdio3 unknow int\r\n");
        sdio3_irq_event_cb(sdio3_irq_arg, SDIO3_IRQ_EVENT_ERR_UNKNOW, NULL);
    }
}
