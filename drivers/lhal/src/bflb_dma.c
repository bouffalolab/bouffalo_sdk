...
{
#ifdef romapi_bflb_dma_channel_deinit
    romapi_bflb_dma_channel_deinit(dev);
#else
    bflb_dma_channel_stop(dev);
...
#if defined(BL616L)
int bflb_dma_channel_lli_insert(struct bflb_device_s *dev, struct bflb_dma_channel_lli_pool_s *lli_pool, uint32_t max_lli_count, struct bflb_dma_channel_lli_transfer_s *transfer, uint32_t count)
{
#ifdef romapi_bflb_dma_channel_lli_insert
    return romapi_bflb_dma_channel_lli_insert(dev, lli_pool, max_lli_count, transfer, count);
#else
    uint32_t channel_base;
    uint32_t actual_transfer_offset = 0;
    uint32_t actual_transfer_len = 0;
    ...
    bflb_dma_feature_control(dev, DMA_CMD_SET_LLI_MUTEX, 1);

    if (bflb_dma_feature_control(dev, DMA_CMD_GET_LLI_MUTEX_STATUS, 0) == DMA_LLI_MUTEX_UNAVAILABLE) {
        return -1;
    }
    ...
    if (count) {
        ...
        if (bflb_dma_feature_control(dev, DMA_CMD_GET_LLI_MUTEX_STATUS, 0) == DMA_LLI_MUTEX_LAST_NODE) {
    ...
    bflb_dma_feature_control(dev, DMA_CMD_SET_LLI_MUTEX, 0);
...
#if (defined(BL606P) || defined(BL808)) && (defined(CPU_M0) || defined(CPU_LP)) || defined(BL616D)
    else if (dev->idx == 1) {
        bflb_irq_attach(dev->irq_num, dma1_isr, NULL);
        bflb_irq_enable(dev->irq_num);
    }
#endif
#if ((defined(BL606P) || defined(BL808)) && defined(CPU_D0)) || defined(BL616D)
    else if (dev->idx == 2) {
