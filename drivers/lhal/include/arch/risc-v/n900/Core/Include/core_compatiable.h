...
{
    uint32_t result;
    result = ((value & 0xff000000) >> 8) | ((value & 0x00ff0000) << 8) | ((value & 0x0000ff00) >> 8) | ((value & 0x000000ff) << 8);
...
__STATIC_INLINE uint64_t __ROR64(uint64_t op1, uint32_t op2)
{
    ...
    return (uint64_t)((tmp1 >> op2) | (tmp1 << (32U - op2))) | ((uint64_t)((tmp2 >> op2) | (tmp2 << (32U - op2))) << 32);
}
