...
*/
__STATIC_INLINE void csi_icache_invalid_range(void *addr, int32_t dsize)
{
#if (__ICACHE_PRESENT == 1U)
    uint32_t linesize = (0x01 << CACHE_INV_ADDR_Pos);
    int32_t op_size = dsize + (uint32_t)addr % linesize;
    uint32_t op_addr = (uint32_t)addr & CACHE_INV_ADDR_Msk;

    __DSB();

    while (op_size >= (linesize * 8)) {
...
__STATIC_INLINE void csi_dcache_enable(void)
{
    ...
    cache = __get_MHCR();
    cache |= (CACHE_MHCR_DE_Msk | CACHE_MHCR_WB_Msk | CACHE_MHCR_WA_Msk | CACHE_MHCR_BPE_Msk | CACHE_MHCR_L0BTB_Msk); /* enable all Cache */
    __set_MHCR(cache);
