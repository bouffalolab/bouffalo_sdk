...
#if 1
/**
 * eloop_event_commit_sync - commmit an event synchronously.
 * @type - event type
 * @request - request buffer
 * @req_len - request length in bytes
 * @resp - response buffer
 * @resp_len - INOUT length of response/buffer
...
/**
 * eloop_event_commit_sync - commmit an event.
 * @type - event type
 * @request - request buffer
 * @req_len - request length in bytes
 */
...
/**
 * eloop_rtos_sock_recv -  counterpart of recv() in eloop_rtos
 * @sock - event type actually, ref eloop_rtos.h
 * @buf - buffer in which caller wants to put result
 * @len - length of caller's buffer.
 * @flags - dummy parameter.
 */
