				       ...
				       union wpa_event_data *data)
{
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_NONE ||
		...
		int this_to = (wpa_s->current_ssid) ? (wpa_s->current_ssid->eapol_1_timeout) : 2;
		wpa_supplicant_req_auth_timeout(wpa_s, this_to ? this_to : 2, 0);
	}
...
#ifdef CONFIG_WPS
		wpas_wps_start_pbc(wpa_s, NULL, 0, 0, 0);
#endif /* CONFIG_WPS */
		break;
