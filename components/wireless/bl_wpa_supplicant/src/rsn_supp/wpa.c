...
int wpa_set_parm_at_connect(wifi_connect_parm_t *parm)
{
    ...
    wpa_set_passphrase(parm->passphrase, (u8 *)parm->ssid.ssid, parm->ssid.len);
    return 0;
}
...
void wpa_set_4way_handshake_timer(u8 quick_conn)
{
    ...
    bl_wifi_timer_stop(&sm->fourway_hs_timer);
    bl_wifi_timer_setfn(&sm->fourway_hs_timer, fourway_hs_timeout_handler_, NULL);
    bl_wifi_timer_start(&sm->fourway_hs_timer, (quick_conn) ?
                      WPA_SUPPLICANT_QUICKCONN_4WAY_HANDSHAKE_TIMEOUT_MS :
                      WPA_SUPPLICANT_4WAY_HANDSHAKE_TIMEOUT_MS);
}
