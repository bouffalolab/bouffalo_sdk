			 ...
			 struct wpa_ie_data *data)
{
	...
	if (hdr->elem_id != WLAN_EID_RSN ||
	    ...
	    WPA_GET_LE16(hdr->version) != RSN_VERSION) {
		  printf("%s: malformed ie or unknown version", __func__);
		return -2;
	}
	...
	if (left >= 2) {
		...
		/* foreach Pairwise Cipher Suite List */
		for (i = 0; i < count; i++) {
		...
		printf("%s: ie too short (for key mgmt)", __func__);
		return -5;
	}
	...
	if (left > 0) {
		//printf("%s: ie has %u trailing bytes - ignored", __func__, left);
	}
