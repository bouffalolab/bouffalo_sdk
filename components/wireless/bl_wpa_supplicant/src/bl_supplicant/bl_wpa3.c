...
static bl_err_t wpa3_build_sae_commit(u8 *bssid, u8 *own_addr, u8 *pw) {
  ...
  if (sae_prepare_commit(own_addr, bssid, pw, strlen((const char *)pw), NULL,
                         &g_sae_data) < 0) {
...
u8 *wpa3_build_sae_msg(u8 *bssid, u8 *mac, u8 *passphrase, u32 sae_msg_type,
                       size_t *sae_msg_len) {
  u8 *buf = NULL;
...
static int wpa3_parse_sae_commit(u8 *buf, u32 len, u16 status) {
  ...
  ret =
      sae_parse_commit(&g_sae_data, buf, len, NULL, 0, (int *)g_allowed_groups);
  if (ret) {
