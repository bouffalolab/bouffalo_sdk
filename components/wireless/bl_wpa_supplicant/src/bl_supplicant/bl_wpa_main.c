...
int wpa_get_key(uint8_t *ifx, int *alg, u8 *addr, int *key_idx, u8 *key,
                size_t key_len, int key_entry_valid) {
...
static void wpa_pkt_free(void *opaque) {
  struct bl_custom_tx_cfm *tx_cfm = opaque;

  if (tx_cfm) {
    if (tx_cfm->cb) {
      tx_cfm->cb(tx_cfm->cb_arg, true);
    }
    free(opaque);
  }
}
...
int wpa_config_bss(wifi_connect_parm_t *parm) {
  ...
  ret = wpa_set_bss(parm->vif_idx, parm->sta_idx, (char *)parm->mac,
                    (char *)parm->bssid, parm->pairwise_cipher,
...
int wpa_parse_wpa_ie_wrapper(const uint8_t *wpa_ie, size_t wpa_ie_len,
                             wifi_wpa_ie_t *data) {
  ...
  data->mgmt_group_cipher =
      cipher_type_map_supp_to_public(ie.mgmt_group_cipher);
