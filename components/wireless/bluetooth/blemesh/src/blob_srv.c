...
static void reset_timer(struct bt_mesh_blob_srv *srv)
{
	...
	#if defined(BFLB_BLE)
	k_delayed_work_submit(&srv->rx_timeout, K_SECONDS(timeout_secs));
	#else
...
static void report_sent(int err, void *cb_data)
{
	...
	#if defined(BFLB_BLE)
    if(srv->pull.report.work_q == NULL)
			     ...
			     struct net_buf_simple *buf)
{
	...
	if (block_count > BT_MESH_BLOB_BLOCKS_MAX) {
		LOG_WRN("Invalid block count (%lu)", block_count);
			      ...
			      struct net_buf_simple *buf)
{
	...
	if (!chunk_size || chunk_size > max_chunk_size(srv) ||
	    (DIV_ROUND_UP((1 << srv->state.xfer.block_size_log), chunk_size) >
	     max_chunk_count(srv))) {
		LOG_WRN("Invalid chunk size: (chunk size: %u, max: %lu, block log: %u, count: %lu)",
				 ...
				 void *cb_arg)
{
	...
	if (block_count_get(srv) > BT_MESH_BLOB_BLOCKS_MAX) {
		LOG_WRN("Loaded block count too high (%lu, max: %u)",
