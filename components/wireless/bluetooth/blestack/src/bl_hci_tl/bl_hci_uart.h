#ifndef __BL_HCI_UART_H
#define __BL_HCI_UART_H
...
typedef void (*bl_hci_rx_ptr)(struct net_buf* rx_buf);
typedef struct net_buf* (*bl_hci_alloc_buf_ptr)(uint8_t rx_type, uint8_t sub_evt, uint16_t len);

typedef struct _bl_hci_trans_callbacks_t
{
  bl_hci_rx_ptr            rx_handler;
  bl_hci_alloc_buf_ptr     alloc_buf;
}bl_hci_trans_callbacks_t;
