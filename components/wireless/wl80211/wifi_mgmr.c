...
int wifi_sta_connect(const char *ssid, const char *key, const char *bssid, const char *akm_str, uint8_t pmf_cfg, uint16_t freq1, uint16_t freq2, uint8_t use_dhcp)
{
  ...
  if ((freq1 && wl80211_freq_valid_check(freq1)) || (freq2 && wl80211_freq_valid_check(freq2)) || (freq1 && freq2 && freq1 != freq2)) {
    return -1;
  } else {
...
#define TEST_CCMP_TKIP                                                         \
  (WL80211_SCAN_AP_RESULT_FLAGS_HAS_CCMP |                                     \
   WL80211_SCAN_AP_RESULT_FLAGS_HAS_TKIP)
