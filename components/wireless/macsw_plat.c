...
int macsw_platform_get_time_us(uint64_t *time_us)
{
    ...
    *time_us = MACSW_PLATFORM_PDS_CNT_TO_US(bflb_rtc_get_time(NULL));
    return 0;
}
#endif
...
void platform_hook_beacon(uint32_t rhd, uint32_t tim, bcn_param_t *param)
{
    ...
    (void) beacon_stamp_us;
    (void) beacon_delay_us;
    ...
    /* update bcn rate */
    uint8_t bcn_rate = _get_leg_rate(param->beacon_rate);
...
int macsw_hook_beacon(const char *f, uintptr_t bcn, uint32_t tim, bcn_param_t *beacon_param) {
  (void)f;
  platform_hook_beacon((uintptr_t)bcn, tim, beacon_param);
  return 0;
}
...
int macsw_hook_evt_disconnected(const char *f) {
  (void)f;
  platform_hook_prevent_sleep(PSM_EVENT_DISCONNECT, 1);
  return 0;
}
#else
int8_t hal_macsw_lp_rssi_restore(void)
