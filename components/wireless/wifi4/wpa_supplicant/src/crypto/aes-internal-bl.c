...
{
    struct bflb_crypto_suite_aes *aes = bflb_crypto_suite_aes_init();
    if (!aes) {
        return NULL;
    }
    bflb_crypto_suite_aes_set_key(aes, AES_ENC, key, len * 8);
    return aes;
}
...
{
    bflb_crypto_suite_aes_crypt((struct bflb_crypto_suite_aes *)ctx, AES_ENC, plain, crypt);
}

void wpa_aes_encrypt_deinit(void *ctx)
{
    void *t = ctx;
    bflb_crypto_suite_aes_deinit((struct bflb_crypto_suite_aes **)&t);
}
...
void *wpa_aes_decrypt_init(const u8 *key, size_t len)
{
    ...
    bflb_crypto_suite_aes_set_key(aes, AES_DEC, key, len * 8);
    return aes;
}

void wpa_aes_decrypt(void *ctx, const u8 *crypt, u8 *plain)
{
    bflb_crypto_suite_aes_crypt((struct bflb_crypto_suite_aes *)ctx, AES_DEC, crypt, plain);
}
