...
lv_obj_t *lv_tabview_create(lv_obj_t *par, const lv_obj_t *copy)
{
    ...
    if (copy == NULL) {
        ...
        if (ancestor_scrl_signal == NULL) {
            ancestor_scrl_signal = lv_obj_get_signal_cb(lv_page_get_scrollable(ext->content));
        }
...
static lv_res_t lv_tabview_signal(lv_obj_t *tabview, lv_signal_t sign, void *param)
{
    ...
    if (sign == LV_SIGNAL_GET_STYLE) {
        ...
        } else if (info->part == LV_TABVIEW_PART_BG_SCROLLABLE)
            info->result = lv_obj_get_state(ext->content,
                                            LV_PAGE_PART_SCROLLABLE);
