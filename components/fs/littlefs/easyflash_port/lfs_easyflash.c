...
ef_get_env_blob_offset(const char *key, void *value_buf, size_t buf_len, size_t *saved_value_len, int offset)
{
    ...
    ret = kv_key2path(path_buffer, sizeof(path_buffer), LFS_EF_NAMESPACE, key);
...
EfErrCode ef_foreach_env(ef_foreach_cb_t cb, void *arg) {
    lfs_dir_t dir = {};
    int ret;
    ...
    while (1) {
        ...
        /* do not use path_buffer here! cb may call ef_get_env_blob,
