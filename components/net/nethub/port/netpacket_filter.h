...
static inline bool npf_is_icmp4(const struct pbuf *skb)
{
    ...
    if (npf_is_ip4(skb)) {
        const struct ip_hdr *iphdr = (const struct ip_hdr *)((uint8_t *)skb->payload + NPF_ETH_HDR_LEN);
...
static inline bool npf_is_dhcp4(const struct pbuf *skb)
{
    ...
    const struct ip_hdr *iphdr = (const struct ip_hdr *)((uint8_t *)skb->payload + NPF_ETH_HDR_LEN);
    const struct udp_hdr *udphdr = (const struct udp_hdr *)((uint8_t *)skb->payload + NPF_ETH_HDR_LEN + NPF_IP4_HDR_LEN);
...
static inline bool npf_is_tcp4_port(const struct pbuf *skb, uint16_t port)
{
    ...
    const struct tcp_hdr *tcphdr = (const struct tcp_hdr *)((uint8_t *)skb->payload + NPF_ETH_HDR_LEN + NPF_IP4_HDR_LEN);
...
static inline bool npf_is_udp4_port(const struct pbuf *skb, uint16_t port)
{
    ...
    const struct udp_hdr *hdr = (const struct udp_hdr *)((uint8_t *)skb->payload + NPF_ETH_HDR_LEN + NPF_IP4_HDR_LEN);
