...
static void netlink_show_help(void)
{
    ...
    printf("  netlink test mtu 1024         - Send 1024 bytes to test MTU\r\n");
#endif
}
...
int cmd_netlink(int argc, char **argv)
{
    ...
    if (strcmp(subcmd, "help") == 0) {
        ...
        if (argc == 3) {
            ...
            if (is_type) {
                ...
                if (total_len > 0) {
                    ...
                    static char g_send_buffer[1024];
        ...
        if (strcmp(test_type, "flow") == 0) {
            ...
            if (count == 0 || size == 0 || size > 1024) {
                printf("[NETLINK_CLI] Error: Invalid count or size (count>0, 1<=size<=1024)\r\n");
                return -1;
            }
            ...
            if (size == 0 || size > 1024) {
                printf("[NETLINK_CLI] Error: Invalid size (1<=size<=1024)\r\n");
                return -1;
            }
