...
- 问题 1：硬件连接并加载 host 驱动后，host 会多出哪些 interface？
  ...
  - **hd_eth0** [必须] - 网卡接口，相当于 STA/AP 转以太网功能，不同平台名称可能会有差异
...
- 问题 6：虚拟出的网卡是ap还是sta呢？

  **自动切换，暂时不支持共存（软件原因，后续可以虚拟多个网卡来支持共存）**。如果开启了AP，默认就会切换到AP通道。做为sta连接了WiFi后，就会充当STA通道，具体切换逻辑device层已经自动完成（底层是通过调用nethub_update_wifichannel接口完成）。
