/**
 * @file bflbvchan_app.c
 * @brief BFLB Virtual Channel Test Application
 */
...
{
    switch (data_type) {
        case BFLBVCHAN_DATA_TYPE_SYSTEM: return "SYSTEM";
        case BFLBVCHAN_DATA_TYPE_WIFI:   return "WIFI";
        case BFLBVCHAN_DATA_TYPE_BLE:    return "BLE";
        case BFLBVCHAN_DATA_TYPE_USER:   return "USER";
        case BFLBVCHAN_DATA_TYPE_AT:     return "AT";
...
static void show_help(void)
{
    ...
    printf("                         Types: system, wifi, ble, user, at\n");
 ...
 *       Type options: system, wifi, ble, user, at (default: user)
 */
static void handle_send(char *args)
{
    ...
    char *type_str = NULL;
    size_t len;
    uint8_t data_type = BFLBVCHAN_DATA_TYPE_USER;  /* Default to user type */

    if (!args) {
        printf("Usage: send [type] <string>\n");
        printf("Types: system, wifi, ble, user, at (default: user)\n");
        return;
    }
    ...
    if (space != NULL) {
        *space = '\0';
        ...
        if (strcmp(type_str, "system") == 0) {
            data_type = BFLBVCHAN_DATA_TYPE_SYSTEM;
        } else if (strcmp(type_str, "wifi") == 0) {
            data_type = BFLBVCHAN_DATA_TYPE_WIFI;
        } else if (strcmp(type_str, "ble") == 0) {
            data_type = BFLBVCHAN_DATA_TYPE_BLE;
        } else if (strcmp(type_str, "user") == 0) {
            data_type = BFLBVCHAN_DATA_TYPE_USER;
        } else if (strcmp(type_str, "at") == 0) {
            data_type = BFLBVCHAN_DATA_TYPE_AT;
        } else {
    ...
    /* Call send interface */
    int ret = bflbvchan_send(data_type, data, len);
    if (ret > 0) {
...
static void handle_test(char *args)
{
    ...
    if (strcmp(test_type, "flow") == 0) {
        ...
        if (count == 0 || size == 0 || size > BFLBVCHAN_MAX_DATA_LEN) {
            printf("Error: Invalid count or size (count>0, 1<=size<=%d)\n", BFLBVCHAN_MAX_DATA_LEN);
            return;
        }
        ...
        for (i = 0; i < count; i++) {
            ...
            int ret = bflbvchan_send(BFLBVCHAN_DATA_TYPE_USER, buffer, (int)size);
            if (ret < 0) {
                failed_count++;
        ...
        if (size == 0 || size > BFLBVCHAN_MAX_DATA_LEN) {
...
static void command_loop(void)
{
    ...
    printf("BFLB Virtual Channel Tool v%s\n", VERSION);
...
int main(int argc, char *argv[])
{
    ...
    /* Initialize BFLB Virtual Channel (singleton, receive thread auto-started) */
    ret = bflbvchan_init();
    if (ret < 0) {
        fprintf(stderr, "Failed to initialize bflbvchan: %d\n", ret);
        fprintf(stderr, "Make sure kernel module is loaded (sudo modprobe hd_netlink)\n");
        return 1;
    }

    /* Register receive callbacks for all data types */
    bflbvchan_register_callback(BFLBVCHAN_DATA_TYPE_SYSTEM, recv_callback);
    bflbvchan_register_callback(BFLBVCHAN_DATA_TYPE_WIFI, recv_callback);
    bflbvchan_register_callback(BFLBVCHAN_DATA_TYPE_BLE, recv_callback);
    bflbvchan_register_callback(BFLBVCHAN_DATA_TYPE_USER, recv_callback);
    bflbvchan_register_callback(BFLBVCHAN_DATA_TYPE_AT, recv_callback);

    /* Enter command loop */
    command_loop();

    /* Cleanup - bflbvchan_deinit() will auto-stop recv thread */
    bflbvchan_deinit();
