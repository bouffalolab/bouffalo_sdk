...
static inline bool npf_is_arp(const struct pbuf *skb)
{
    ...
    return hdr->type == PP_HTONS(ETHTYPE_ARP);
}
...
static inline bool npf_is_8021X(const struct pbuf *skb)
{
    ...
    return hdr->type == PP_HTONS(ETHTYPE_8021X);
}
...
static inline bool npf_is_ip4(const struct pbuf *skb)
{
    ...
    return hdr->type == PP_HTONS(ETHTYPE_IP);
}
...
static inline bool npf_is_icmp4(const struct pbuf *skb)
{
    ...
    if (npf_is_ip4(skb)) {
        const struct ip_hdr *iphdr =
            (const struct ip_hdr *)((uint8_t *)skb->payload + NPF_ETH_HDR_LEN);

        return IPH_PROTO(iphdr) == IP_PROTO_ICMP;
    }
...
static inline bool npf_is_dhcp4(const struct pbuf *skb)
{
    ...
    srcport = lwip_ntohs(udphdr->src);
    dstport = lwip_ntohs(udphdr->dest);
...
static inline bool npf_is_tcp4_port(const struct pbuf *skb, uint16_t port)
{
    ...
    return lwip_htons(port) == tcphdr->src || lwip_htons(port) == tcphdr->dest;
}
...
static inline bool npf_is_udp4_port(const struct pbuf *skb, uint16_t port)
{
    ...
    hdr = (const struct udp_hdr *)((uint8_t *)skb->payload + NPF_ETH_HDR_LEN + NPF_IP4_HDR_LEN);
    return lwip_htons(port) == hdr->src || lwip_htons(port) == hdr->dest;
}
