...
_ssize_t _write_tty_r(struct _reent *reent, int fd, const void *ptr, size_t size)
{
    if (fd == 0x3ff) {
        ...
        /* Debug: count special characters */
        size_t cr_count = 0;
        size_t lf_count = 0;
...
#ifdef CONFIG_CONSOLE_WO
            bflb_wo_uart_putchar(console, ch);
#else
            bflb_uart_putchar(console, ch);
#endif
        }
        /* TEMP: Return the actual size to indicate success */
        (void)cr_count;
        (void)lf_count;
        return size;
    } else {
