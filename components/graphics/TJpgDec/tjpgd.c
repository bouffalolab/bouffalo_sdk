...
static TJPGDEC_ATTRIBUTE_DATA const uint8_t Zig[64] = {	/* Zigzag-order to raster-order conversion table */
...
static TJPGDEC_ATTRIBUTE_DATA const uint16_t Ipsf[64] = {	/* See also aa_idct.png */
...
static TJPGDEC_ATTRIBUTE_DATA const uint8_t Clip8[1024] = {
	/* 0..255 */
		...
		if (JD_FORMAT != 2) {	/* RGB output (build an RGB MCU from Y/C component) */
            for (iy = 0; iy < my; iy++) {
                ...
                for (ix = 0; ix < mx; ix++) {
                    ...
                    if (JD_YUV444_MODE) {
                        *pix++ = BYTECLIP(yy); //yy > 255 ? 255 : (yy < 0 ? 0 : yy);
                        *pix++ = BYTECLIP(cb); //cr > 255 ? 255 : (cr < 0 ? 0 : cr);
                        *pix++ = BYTECLIP(cr); //cb > 255 ? 255 : (cb < 0 ? 0 : cb);
                    } else {
                        cb -= 128;
                        cr -= 128;
