...
//#include "bflb_yoc_sys.h"
// #include "bflb_yoc_dma.h"
// #include "ulog/ulog.h"
...
static  uint32_t bflb_yoc_dmaAddr[DMA_ID_MAX] = { DMA0_BASE, DMA1_BASE, DMA2_BASE };

static _dma_callbak_func_t dmaIntCbfArra[DMA_ID_MAX][DMA_CH_MAX][DMA_INT_ALL];

csi_dma_t bflb_yoc_dma_dev[DMA_ID_MAX];
...
static void __dma_inthandler(DMA_ID_Type dmaId, void *arg)
{
    ...
    uint8_t ch;
    /* Get DMA register */
    uint32_t dma_ch = bflb_yoc_dmaAddr[dmaId];
...
void msp_dma_irq_function(int dmaId, int dmaChan, void *irq_handler, void *dev)
{
    ...
    switch (dmaId) {
        case DMA0_ID:
            csi_irq_attach(DMA0_ALL_IRQn, __dma0_irqhandlert, (void *)&bflb_yoc_dma_dev[dmaId]);
            csi_irq_enable(DMA0_ALL_IRQn);
            break;
        case DMA1_ID:
            csi_irq_attach(DMA1_ALL_IRQn, __dma1_irqhandlert, (void *)&bflb_yoc_dma_dev[dmaId]);
