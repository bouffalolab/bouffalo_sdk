...
#ifndef CONFIG_CODEC_USE_I2S_TX 
...
#endif
#ifndef CONFIG_CODEC_USE_I2S_RX
...
#if (defined CONFIG_CODEC_USE_I2S_RX) || (defined CONFIG_CODEC_USE_ES8388)
...
{
    //bflb_yoc_audio_input_set_mute(ch, enable);
...
{
    //bflb_yoc_audio_input_set_mix_gain(ch, 0, val);
    //bl606p not support
...
{
    //bflb_yoc_audio_input_digital_gain(ch, 1, val);
...
{
    //bflb_yoc_audio_input_analog_gain(ch, val);
...
#if (defined CONFIG_CODEC_USE_I2S_TX) || (defined CONFIG_CODEC_USE_ES8388)
    msp_i2s_device_init(config->sample_rate);
#else
    msp_codec_pin_cfg_t pin_cfg = msp_codec_pin_config();
...
{
    //bflb_yoc_audio_output_close(ch);
    if (tx_context.dma) {
...
{
    //bflb_yoc_audio_output_set_mute(ch, enable ? 0 : 1);
...
{
    //bflb_yoc_audio_output_digital_gain(ch, 1, val);
...
{
    //bflb_yoc_audio_output_analog_gain(ch, val);
    return XCODEC_OK;
}
