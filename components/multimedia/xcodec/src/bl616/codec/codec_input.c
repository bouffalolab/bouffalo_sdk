...
#if 0
    bflb_yoc_dma_set_clk(dma_id, dma_ch);
    bflb_yoc_dma_enable(dma_id);
    bflb_yoc_dma_channel_disable(dma_id, dma_ch);

    DMA_LLI_Init(dma_id, dma_ch, (DMA_LLI_Cfg_Type *)&lli_cfg_dma);
    DMA_LLI_Update(dma_id, dma_ch, (uint32_t)(&(context->dma->node[0].dma_cfg)));
    DMA_IntMask(dma_id, dma_ch, DMA_INT_ALL, MASK);

    bflb_yoc_dma_irq_function(dma_id, dma_ch, &_aui_rx_dma_irq_hander, (void *)(context));
