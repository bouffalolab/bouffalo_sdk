...
struct net_device *mr_sdio_eth_netdev_init(struct mr_msg_ctrl *msg_ctrl, uint8_t msg_tag)
{
    ...
    netdev = alloc_netdev(sizeof(struct mr_sdio_eth_priv), ETH_DRV_NAME, NET_NAME_UNKNOWN, ether_setup);
#else
    netdev = alloc_netdev(sizeof(struct mr_sdio_eth_priv), ETH_DRV_NAME, ether_setup);
#endif
    if (!netdev) {
