/**
 * @file mr_sdio_manage.h
 * @brief SDIO management layer header definitions
...
#ifndef __MR_SDIO_MANAGE_H__
#define __MR_SDIO_MANAGE_H__
 ...
 */
typedef void (*card2host_irq_cb)(struct mr_sdio_manage *sdio_manage);
...
struct mr_sdio_card {
    ...
    struct net_device *netdev_wifi;          /**< Associated network device */
    struct net_device *netdev_wifi_sta;      /**< Associated network device */
    struct net_device *netdev_wifi_ap;       /**< Associated network device */
    struct net_device *netdev_eth;           /**< Associated network device */
    struct mr_netlink_priv *netlink;         /**< Associated netlink private structure */
    struct mr_virtualchan_priv *virtualchan; /**< Associated virtuachan private structure */
    ...
    struct mr_tty_priv *tty_msg_user_3;
    /** @} */
    ...
    int (*sdio_read_reg)(struct mr_sdio_card *card, uint32_t reg, uint8_t *data);
    /** Write to function register */
    int (*sdio_write_reg)(struct mr_sdio_card *card, uint32_t reg, uint8_t data);
    /** @} */
    ...
    int (*sdio_read_fn0_reg)(struct mr_sdio_card *card, uint32_t reg, uint8_t *data);
    /** Write to function 0 register */
    int (*sdio_write_fn0_reg)(struct mr_sdio_card *card, uint32_t reg, uint8_t data);
    /** @} */
    ...
    int (*sdio_read_custom_reg)(struct mr_sdio_card *card, uint16_t reg_offset, void *buff, uint16_t len);
    /** Write to custom register (SDIO3 only) */
    int (*sdio_write_custom_reg)(struct mr_sdio_card *card, uint16_t reg_offset, void *buff, uint16_t len);
    /** @} */
    ...
    int (*sdio_upld_transf)(struct mr_sdio_card *card, struct sdio_trans_desc *upld_desc, int retry_cnt);
    /** Download (host to device) data transfer */
    int (*sdio_dnld_transf)(struct mr_sdio_card *card, struct sdio_trans_desc *dnld_desc, int retry_cnt);
    /** @} */
    ...
    int (*sdio_upld_irq_register)(struct mr_sdio_card *card, card2host_irq_cb upld_irq_cb);
...
struct mr_sdio_manage {
     ...
     * @{
     */
    struct mr_sdio_card *sdio_card; /**< Associated SDIO card structure */
    struct mr_msg_ctrl *msg_ctrl;   /**< Associated message control structure */
    /** @} */
};
 ...
 * @retval <0 Error code
 */
int mr_sdio_manage_init(struct mr_sdio_manage **sdio_manage, struct mr_sdio_card *sdio_card);
