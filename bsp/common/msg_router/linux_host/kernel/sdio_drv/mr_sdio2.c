...
/**
 * @file mr_sdio2.c
 * @brief SDIO2 Interface Driver Implementation
 * @details This module implements the SDIO2 interface driver.
 *          It provides low-level SDIO communication,
 ...
 */
static int mr_sdio2_read_reg(struct mr_sdio_card *card, uint32_t reg, uint8_t *data)
{
    int ret = -1;
    uint8_t val;
 ...
 *          with proper host claiming and error handling.
 */
static int mr_sdio2_write_reg(struct mr_sdio_card *card, uint32_t reg, uint8_t data)
{
    int ret;
 ...
 */
static int mr_sdio2_read_fn0_reg(struct mr_sdio_card *card, uint32_t reg, uint8_t *data)
{
    int ret = -1;
    uint8_t val;
 ...
 */
static int mr_sdio2_write_fn0_reg(struct mr_sdio_card *card, uint32_t reg, uint8_t data)
{
    int ret;
 ...
 */
static int mr_sdio2_upld_transmit(struct mr_sdio_card *card, struct sdio_trans_desc *upld_desc, int retry_cnt)
{
    int ret;
    int trans_len;
    int try_cnt = 0;
    ...
    while ((card->sdio2_upld_bitmap & (0x01 << upld_port)) == 0) {
        ...
        if (upld_port < 8) {
            ...
            ret = mr_sdio2_read_reg(card, SDIO2_RD_BIT_MAP_OFFSET, &bitmap_byte);
            if (ret < 0) {
            ...
            ret = mr_sdio2_read_reg(card, SDIO2_RD_BIT_MAP_OFFSET + 1, &bitmap_byte);
            if (ret < 0) {
    ...
    if (card->rd_len_compress) {
        /* Read first byte */
        ret = mr_sdio2_read_reg(card, SDIO2_PORTn_UPLD_LEN_OFFSET(upld_port), &reg_temp[0]);
        if (ret < 0) {
            goto return_out;
        }
        /* 2-byte mode, read second byte */
        if (reg_temp[0] & 0x01) {
            ret = mr_sdio2_read_reg(card, SDIO2_PORTn_UPLD_LEN_OFFSET(upld_port) + 1, &reg_temp[1]);
            if (ret < 0) {
                goto return_out;
            }
        }
 ...
 */
static int mr_sdio2_dnld_transmit(struct mr_sdio_card *card, struct sdio_trans_desc *dnld_desc, int retry_cnt)
{
    int ret;
    int trans_len;
    int try_cnt = 0;
    ...
    while ((card->sdio2_dnld_bitmap & (0x01 << dnld_port)) == 0) {
        ...
        if (dnld_port < 8) {
            /* Get bitmap lower byte */
            ret = mr_sdio2_read_reg(card, SDIO2_WR_BIT_MAP_OFFSET, &bitmap_byte);
            if (ret < 0) {
            ...
            /* Get bitmap upper byte */
            ret = mr_sdio2_read_reg(card, SDIO2_WR_BIT_MAP_OFFSET + 1, &bitmap_byte);
            if (ret < 0) {
 ...
 */
static void mr_sdio2_card2host_irq(struct sdio_func *func)
{
    int ret;
    ...
    ret = mr_sdio2_write_reg(card, SDIO2_HOST_INT_STATUS_OFFSET, 0);
    if (ret < 0) {
        return;
    }
 ...
 */
static int mr_sdio2_upld_irq_register(struct mr_sdio_card *card, card2host_irq_cb upld_irq_cb)
{
    int ret;
    ...
    if (upld_irq_cb) {
        ...
        ret = mr_sdio2_write_reg(card, SDIO2_HOST_INT_MASK_OFFSET, SDIO2_HOST_INT_UMSK_UPLD);
        if (ret < 0) {
            return ret;
        }
        /* Enable card-to-host interrupt */
        sdio_claim_host(card->func);
        ret = sdio_claim_irq(card->func, mr_sdio2_card2host_irq);
        sdio_release_host(card->func);
        if (ret < 0) {
        ...
        ret = mr_sdio2_write_reg(card, SDIO2_HOST_INT_MASK_OFFSET, 0);
        if (ret < 0) {
            return ret;
        }
    }
 ...
 */
static int mr_sdio2_enable_func(struct mr_sdio_card *card)
{
    int ret;

    SDIO_DRV_INFO(card, "mr_sdio2_enable_func\n");
 ...
 */
static int mr_sdio2_disable_func(struct mr_sdio_card *card)
{
    int ret;
    SDIO_DRV_INFO(card, "mr_sdio2_disable_func\n");
 ...
 */
static int mr_sdio2_set_block_size(struct mr_sdio_card *card, uint16_t size)
{
    int ret;

    SDIO_DRV_INFO(card, "mr_sdio2_set_block_size  %d -> %d\n", card->func->cur_blksize, size);
 ...
 */
static void mr_sdio2_info_dump(struct mr_sdio_card *card)
{
    /* dump queue info */
 ...
 */
static int mr_sdio2_info_update(struct mr_sdio_card *card)
{
    int ret = 0;
    uint8_t io_reg;
    uint32_t ioport_base = 0;

    SDIO_DRV_INFO(card, "mr_sdio2_info_update\n");

    /* 1. read io port offset */
    /* io port byte 0 */
    ret = mr_sdio2_read_reg(card, SDIO2_IO_PORT_OFFSET, &io_reg);
    if (ret < 0) {
        goto return_out;
    }
    ioport_base |= (uint32_t)io_reg;
    /* io port byte 1 */
    ret = mr_sdio2_read_reg(card, SDIO2_IO_PORT_OFFSET + 1, &io_reg);
    if (ret < 0) {
        goto return_out;
    }
    ioport_base |= (uint32_t)io_reg << 8;
    /* io port byte 2 */
    ret = mr_sdio2_read_reg(card, SDIO2_IO_PORT_OFFSET + 2, &io_reg);
    if (ret < 0) {
        goto return_out;
    }
    ...
    ret = mr_sdio2_read_reg(card, SDIO2_SCRATCH0_1_OFFSET, &io_reg);
    if (ret < 0) {
        goto return_out;
    }
    if (io_reg) {
    ...
    ret = mr_sdio2_read_reg(card, SDIO2_SCRATCH0_2_OFFSET, &io_reg);
    if (ret < 0) {
        goto return_out;
    }
    ...
    ret = mr_sdio2_read_reg(card, SDIO2_SCRATCH0_3_OFFSET, &io_reg);
    if (ret < 0) {
        goto return_out;
    }
 ...
 */
static int mr_sdio2_wait_card_ready(struct mr_sdio_card *card, int timeout_ms)
{
    int ret;
    uint8_t io_reg;
    ktime_t time_s;
 ...
 */
static int mr_sdio2_host_ready(struct mr_sdio_card *card)
{
    int ret;

    ret = mr_sdio2_write_reg(card, SDIO2_SCRATCH0_0_OFFSET, SDIO2_HOST_UPLD_SIZE_MAX / SDIO2_SIZE_CONSULT_MULTIPLE);
    if (ret < 0) {
 ...
 */
static int mr_sdio2_clean_ready(struct mr_sdio_card *card)
{
    int ret;

    ret = mr_sdio2_write_reg(card, SDIO2_SCRATCH0_0_OFFSET, 0);
    if (ret < 0) {
        SDIO_DRV_ERR(card, "clean host_ready error\n");
        return ret;
    }

    ret = mr_sdio2_write_reg(card, SDIO2_SCRATCH0_1_OFFSET, 0);
    if (ret < 0) {
 ...
 */
static int mr_sdio2_init(struct mr_sdio_card *card)
{
    int ret;
    ...
    /**
     * @brief Manual block size setting for SDU func1
     * @details SDU func1 lacks cistpl_funce block which prevents automatic setting,
     *          so manual configuration is required here
     */
    ...
    /* 2. set block size */
    ret = mr_sdio2_set_block_size(card, SDIO2_HOST_BLOCK_SIZE);
    if (ret < 0) {
    ...
    ret = mr_sdio2_wait_card_ready(card, 100);
    if (ret == -ETIME) {
    ...
    /* */
    card->sdio_read_reg = mr_sdio2_read_reg;
    card->sdio_write_reg = mr_sdio2_write_reg;
    /* */
    card->sdio_read_fn0_reg = mr_sdio2_read_fn0_reg;
    card->sdio_write_fn0_reg = mr_sdio2_write_fn0_reg;
    /* */
    card->sdio_read_custom_reg = NULL;
    card->sdio_write_custom_reg = NULL;
    /* */
    card->sdio_upld_transf = mr_sdio2_upld_transmit;
    card->sdio_dnld_transf = mr_sdio2_dnld_transmit;
    /* */
    card->sdio_upld_irq_register = mr_sdio2_upld_irq_register;
 ...
 */
static int mr_sdio2_boot_reinit(struct mr_sdio_card *card)
{
    int ret;
    ...
    ret = mr_sdio2_wait_card_ready(card, 200);
    if (ret < 0) {
        SDIO_DRV_ERR(card, "failed to wait_card_ready\n");
        goto exit;
    }
 ...
 */
static void mr_sdio2_remove(struct sdio_func *func)
{
...
#ifdef CONFIG_MR_NETDEV
#ifdef CONFIG_MR_NETDEV_WIFI
    if (card->netdev_wifi) {
        /* stop netdev_wifi*/
        mr_sdio_eth_netdev_deinit(card->netdev_wifi);
        card->netdev_wifi= NULL;
    }
#endif
#ifdef CONFIG_MR_NETDEV_WIFI_STA
    if (card->netdev_wifi_sta) {
        /* stop netdev_wifi_sta */
        mr_sdio_eth_netdev_deinit(card->netdev_wifi_sta);
        card->netdev_wifi_sta = NULL;
    }
#endif
#ifdef CONFIG_MR_NETDEV_WIFI_AP
    if (card->netdev_wifi_ap) {
        /* stop netdev_wifi_ap */
        mr_sdio_eth_netdev_deinit(card->netdev_wifi_ap);
        card->netdev_wifi_ap = NULL;
    }
#endif
#ifdef CONFIG_MR_NETDEV_ETH
    if (card->netdev_eth) {
        /* stop netdev_eth */
        mr_sdio_eth_netdev_deinit(card->netdev_eth);
        card->netdev_eth = NULL;
    }
#endif
#endif
    ...
    if (card->virtualchan) {
        /* stop virtualchan */
        mr_virtualchan_deinit(card->virtualchan);
        card->virtualchan= NULL;
    }
#endif
 ...
 */
static int mr_sdio2_probe(struct sdio_func *func, const struct sdio_device_id *id)
{
    int ret = 0;
    ...
    ret = mr_speed_test_run(msg_ctrl, MR_MSG_TAG_TEST_1, true);
    if (ret < 0) {
        goto msg_ctrl_exit;
    }
    ret = mr_speed_test_run(msg_ctrl, MR_MSG_TAG_TEST_2, true);
    if (ret < 0) {
        goto msg_ctrl_exit;
    }
    ret = mr_speed_test_run(msg_ctrl, MR_MSG_TAG_TEST_3, true);
    if (ret < 0) {
        goto msg_ctrl_exit;
    }
#endif
...
#ifdef CONFIG_MR_NETDEV_WIFI
    /* create netdev_wifi eth */
    card->netdev_wifi = mr_sdio_eth_netdev_init(msg_ctrl, MR_MSG_TAG_NETDEV_WIFI_STA);
    if (card->netdev_wifi == NULL) {
        SDIO_DRV_ERR(card, "failed to create netdev_wifi\n");
        ret = -ENOMEM;
        goto msg_ctrl_exit;
    }
#endif
...
#ifdef CONFIG_MR_NETDEV_WIFI_STA
    /* create netdev_wifi_sta eth */
    card->netdev_wifi_sta = mr_sdio_eth_netdev_init(msg_ctrl, MR_MSG_TAG_NETDEV_WIFI_STA);
    if (card->netdev_wifi_sta == NULL) {
        SDIO_DRV_ERR(card, "failed to create netdev_wifi_sta\n");
        ret = -ENOMEM;
        goto msg_ctrl_exit;
    }
#endif
...
#ifdef CONFIG_MR_NETDEV_WIFI_AP
    /* create netdev_wifi_sta eth */
    card->netdev_wifi_ap = mr_sdio_eth_netdev_init(msg_ctrl, MR_MSG_TAG_NETDEV_WIFI_AP);
    if (card->netdev_wifi_ap == NULL) {
        SDIO_DRV_ERR(card, "failed to create netdev_wifi_ap\n");
        ret = -ENOMEM;
        goto msg_ctrl_exit;
    }
#endif
...
#ifdef CONFIG_MR_NETDEV_ETH
    /* create netdev_eth */
    card->netdev_eth = mr_sdio_eth_netdev_init(msg_ctrl, MR_MSG_TAG_NETDEV_ETH);
    if (card->netdev_eth == NULL) {
        SDIO_DRV_ERR(card, "failed to create netdev_eth\n");
        ret = -ENOMEM;
        goto msg_ctrl_exit;
    }
#endif
...
#ifdef CONFIG_MR_VIRTUALCHAN
    /* create virtualchan */
    card->virtualchan = mr_virtualchan_init(msg_ctrl, MR_MSG_TAG_VIRTUALCHAN);
    if (card->virtualchan == NULL) {
        SDIO_DRV_ERR(card, "failed to create virtualchan\n");
        ret = -ENOMEM;
        goto msg_ctrl_exit;
    }
#endif
...
#ifdef CONFIG_MR_TTY_CMD
    /* create tty devices */
    card->tty_msg_cmd = mr_tty_init(msg_ctrl, MR_MSG_TAG_TTY_CMD, "ttyAT", 0);
...
exit:
    if (ret < 0) {
        ...
        mr_sdio2_upld_irq_register(card, NULL);
        /* disable func */
        mr_sdio2_disable_func(card);
        kfree(card);
    }
...
static const struct sdio_device_id mr_sdio2_ids[] = {
...
struct sdio_driver mr_sdio2 = {
    .name = "mr_sdio2",
    .id_table = mr_sdio2_ids,
    .probe = mr_sdio2_probe,
    .remove = mr_sdio2_remove,
};
