/**
 * @file mr_msg_ctrl.c
 * @brief Message Controller for SDIO Communication
 * @details This module implements a unified message routing system for data flow management
 *          across multiple transport interfaces (SDIO). It uses TLV (Type-Length-Value)
 ...
 */
static int mr_msg_upld_recv_handler(struct mr_msg_ctrl *msg_ctrl, struct sk_buff *skb)
{
    int ret = 0;
    ...
    upld_recv_cb = msg_ctrl->mr_msg_upld_recv_cb[msg_tag];
    upld_recv_arg = msg_ctrl->mr_msg_upld_recv_arg[msg_tag];
    ...
    if (ret >= 0 && msg_tag < MR_MSG_TAG_MAX) {
 ...
 */
static int mr_msg_upld_recv_cplt_cb(struct sk_buff *skb, bool success, void *arg)
{
    struct mr_msg_ctrl *msg_ctrl = arg;

    MSG_CTRL_DBG(msg_ctrl, "mr_msg_upld_recv_cplt_cb, success: %s", success ? "true" : "false");
    ...
    if (skb_queue_len(&msg_ctrl->upld_skb_head) >= MR_MSG_CTRL_UPLD_QUEUE_DEPTH_MAX) {
 ...
 */
static int mr_msg_dnld_send_handler(struct mr_msg_ctrl *msg_ctrl, struct sk_buff *skb)
{
    int ret = 0;
    ...
    dnld_send_arg = msg_ctrl->mr_msg_dnld_send_arg[msg_tag];
 ...
 */
static void mr_msg_workqueue_func(struct work_struct *work)
{
    struct mr_msg_ctrl *msg_ctrl = container_of(work, struct mr_msg_ctrl, recv_work);
    struct sk_buff *skb = NULL;
    bool upld_proc_flag = false;
    bool dnld_proc_flag = false;
    int processed = 0;

    MSG_CTRL_DBG(msg_ctrl, "mr_msg_workqueue_func");

    while (1) {
        skb = skb_dequeue(&msg_ctrl->upld_skb_head);
        if (skb) {
            upld_proc_flag = false;
            mr_msg_upld_recv_handler(msg_ctrl, skb);
        } else {
            upld_proc_flag = true;
        }
        ...
        if (skb) {
            ...
            mr_msg_dnld_send_handler(msg_ctrl, skb);
        } else {
            dnld_proc_flag = true;
        }
 ...
 */
static int mr_msg_sdio_send(struct mr_sdio_manage *sdio_manage, struct sk_buff *skb)
{
    int ret;

    ret = mr_sdio_manage_dnld_send(sdio_manage, skb);
    return ret;
}
 ...
 */
static int mr_msg_usb_send(void *usb_manage, struct sk_buff *skb)
{
    return -1;
}
 ...
 */
static int mr_msg_ctrl_del(struct mr_msg_ctrl *msg_ctrl)
{
    int i;
    ...
    for (i = 0; i < MR_MSG_TAG_MAX; i++) {
        msg_ctrl->mr_msg_upld_recv_cb[i] = NULL;
        msg_ctrl->mr_msg_upld_recv_arg[i] = NULL;
        msg_ctrl->mr_msg_dnld_send_cb[i] = NULL;
        msg_ctrl->mr_msg_dnld_send_arg[i] = NULL;
    }
 ...
 */
static struct mr_msg_ctrl *mr_msg_ctrl_create(void)
{
    struct mr_msg_ctrl *msg_ctrl;

    msg_ctrl = kzalloc(sizeof(struct mr_msg_ctrl), GFP_KERNEL);
    if (!msg_ctrl) {
        return NULL;
    }
    ...
    /* msg work queue */
    msg_ctrl->recv_workqueue = alloc_workqueue("MR_MSG_RECV_WQ", WQ_HIGHPRI | WQ_UNBOUND | WQ_MEM_RECLAIM, 1);
    if (!msg_ctrl->recv_workqueue) {
        goto err_exit;
    }
...
int mr_sdio_msg_ctrl_deinit(struct mr_msg_ctrl *msg_ctrl)
{
    ...
    if (msg_ctrl->sdio_manage) {
        ...
        mr_sdio_manage_cplt_cb_register(msg_ctrl->sdio_manage, NULL, NULL, NULL, NULL);

        msg_ctrl->sdio_manage->msg_ctrl = NULL;
    }

    MSG_CTRL_INFO(msg_ctrl, "mr_sdio_msg_ctrl_deinit");

    /* remove workqueue */
    mr_msg_ctrl_del(msg_ctrl);
 ...
 * @param mr_msg_ctrl Pointer to message controller pointer (output)
...
int mr_sdio_msg_ctrl_init(struct mr_msg_ctrl **mr_msg_ctrl, struct mr_sdio_manage *sdio_manage)
{
    ...
    if (!mr_msg_ctrl || !sdio_manage) {
        return -EINVAL;
    }

    msg_ctrl = mr_msg_ctrl_create();
    if (!msg_ctrl) {
        return -ENOMEM;
    }

    /* register both upload and download callbacks */
    mr_sdio_manage_cplt_cb_register(sdio_manage, mr_msg_upld_recv_cplt_cb, msg_ctrl, mr_msg_dnld_send_cplt_cb,
                                    msg_ctrl);
 ...
 * @param mr_msg_ctrl Pointer to message controller structure
 * @retval 0 Success
 */
                       ...
                       mr_msg_cb_t dnld_send_cb, void *dnld_send_arg)
{
    ...
    if (upld_recv_cb) {
        ...
        if (msg_ctrl->mr_msg_upld_recv_cb[tag] != NULL) {
            mutex_unlock(&msg_ctrl->upld_cb_mutex);
            MSG_CTRL_ERR(msg_ctrl, "msg upld recv cb already registered for tag: %d", tag);
            return -EEXIST;
        }

        msg_ctrl->mr_msg_upld_recv_cb[tag] = upld_recv_cb;
        msg_ctrl->mr_msg_upld_recv_arg[tag] = upld_recv_arg;
    ...
    if (dnld_send_cb) {
        ...
        if (msg_ctrl->mr_msg_dnld_send_cb[tag] != NULL) {
            mutex_unlock(&msg_ctrl->dnld_cb_mutex);
            MSG_CTRL_ERR(msg_ctrl, "msg dnld send cb already registered for tag: %d", tag);
            return -EEXIST;
        }

        msg_ctrl->mr_msg_dnld_send_cb[tag] = dnld_send_cb;
        msg_ctrl->mr_msg_dnld_send_arg[tag] = dnld_send_arg;
...
int mr_msg_cb_unregister(struct mr_msg_ctrl *msg_ctrl, uint8_t tag)
{
    ...
    mutex_lock(&msg_ctrl->upld_cb_mutex);
    msg_ctrl->mr_msg_upld_recv_cb[tag] = NULL;
    msg_ctrl->mr_msg_upld_recv_arg[tag] = NULL;
    mutex_unlock(&msg_ctrl->upld_cb_mutex);

    /* Unregister download send callback */
    mutex_lock(&msg_ctrl->dnld_cb_mutex);
    msg_ctrl->mr_msg_dnld_send_cb[tag] = NULL;
    msg_ctrl->mr_msg_dnld_send_arg[tag] = NULL;
    mutex_unlock(&msg_ctrl->dnld_cb_mutex);
...
int mr_msg_ctrl_send(struct mr_msg_ctrl *msg_ctrl, struct sk_buff *skb)
{
    ...
    if (msg_ctrl->msg_hw_mode == 0) {
        ret = mr_msg_sdio_send(msg_ctrl->sdio_manage, skb);
    } else {
        ret = mr_msg_usb_send(msg_ctrl->usb_manage, skb);
    }
    if (ret < 0) {
...
err_exit:
    ...
    MSG_CTRL_ERR(msg_ctrl, "mr_msg_sdio_send failed: %d", ret);
    return ret;
}
...
void mr_msg_ctrl_dump_stats(struct mr_msg_ctrl *msg_ctrl)
{
    ...
    MSG_CTRL_INFO(msg_ctrl, "RX Errors: %lu, TX Errors: %lu, RX Dropped: %lu\n", msg_ctrl->stats.rx_errors,
                  msg_ctrl->stats.tx_errors, msg_ctrl->stats.rx_dropped);
    ...
    for (i = 0; i < MR_MSG_TAG_MAX; i++) {
        if (msg_ctrl->stats.rx_packets[i] > 0 || msg_ctrl->stats.tx_packets[i] > 0) {
            MSG_CTRL_INFO(msg_ctrl, "Tag %d: RX %lu packets/%lu bytes, TX %lu packets/%lu bytes\n", i,
                          msg_ctrl->stats.rx_packets[i], msg_ctrl->stats.rx_bytes[i], msg_ctrl->stats.tx_packets[i],
                          msg_ctrl->stats.tx_bytes[i]);
        }
    }

    /* Show queue status */
    MSG_CTRL_INFO(msg_ctrl, "Current upld queue length: %u/%d\n", skb_queue_len(&msg_ctrl->upld_skb_head),
                  MR_MSG_CTRL_UPLD_QUEUE_DEPTH_MAX);
