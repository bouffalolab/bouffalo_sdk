/**
 * @file mr_debugfs.c
 * @brief DebugFS Interface Implementation
 ...
 */
static ssize_t mr_dbgfs_sdiord_read(struct file *file,
                                    char __user *user_buf,
                                    size_t count, loff_t *ppos)
{
    struct mr_sdio_card *card = file->private_data;
    ssize_t read;
    char recvbuf[256] = { 0 };
    int port1 = 0x10001;

    //TODO: read data from sdio
    mr_read_data_sync(card, recvbuf, 256, port1);
 ...
 */
static ssize_t mr_dbgfs_sdiowr_write(struct file *file,
                                     ...
                                     size_t count, loff_t *ppos)
{
    ...
    mr_write_data_sync(card, sendbuf, 256, port);
