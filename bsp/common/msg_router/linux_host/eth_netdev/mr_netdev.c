...
static int mr_eth_upld_recv_net_data(struct net_device *netdev, struct sk_buff *skb)
{
    ...
    skb_pull(skb, sizeof(struct mr_eth_msg_packt));
...
static int mr_eth_upld_recv_cb(struct sk_buff *skb, bool success, void *arg)
{
    ...
    struct mr_sdio_eth_priv *priv = netdev_priv(netdev);
    struct mr_eth_msg_packt *eth_msg_packt;

    /* msg_ctrl has already validated packet size and trimmed excess data */
    eth_msg_packt = (struct mr_eth_msg_packt *)skb->data;

    /* Fast path for data packets in RUN state to avoid waking up the kthread */
    if (priv->eth_status == MR_NETDEV_HSTA_DEVICE_RUN &&
        (eth_msg_packt->flag == MR_NETDEV_FLAG_UPLD_DATA || eth_msg_packt->flag == MR_NETDEV_FLAG_CREDIT_UPDATE)) {
        /* Update flow control, must be protected with lock */
        if (eth_msg_packt->credit_update_flag) {
            mr_eth_credit_limit_update(netdev, eth_msg_packt->credit_limit_cnt);
        }

        if (eth_msg_packt->flag == MR_NETDEV_FLAG_UPLD_DATA) {
...
static int mr_eth_dnld_send_cb(struct sk_buff *skb, bool success, void *arg)
{
    ...
    data_len = eth_msg_packt->msg_packt.len - (sizeof(struct mr_eth_msg_packt) - sizeof(struct mr_msg_packt));

    if (eth_msg_packt->flag == MR_NETDEV_FLAG_DNLD_DATA) {
        if (success) {
...
static netdev_tx_t mr_eth_netdev_start_xmit(struct sk_buff *skb, struct net_device *netdev)
{
    ...
    /* Check skb length */
    if (skb->len + sizeof(struct mr_eth_msg_packt) > priv->msg_ctrl->msg_dnld_max_size) {
    ...
    if (skb_headroom(skb) < sizeof(struct mr_eth_msg_packt)) {
        struct sk_buff *new_skb = skb_realloc_headroom(skb, sizeof(struct mr_eth_msg_packt));
        if (!new_skb) {
    ...
    eth_msg_packt = (struct mr_eth_msg_packt *)skb_push(skb, sizeof(struct mr_eth_msg_packt));
    eth_msg_packt->msg_packt.tag = priv->msg_tag;
    eth_msg_packt->msg_packt.sub_tag = 0;
    eth_msg_packt->msg_packt.len = skb->len - sizeof(struct mr_msg_packt); /* Length is pure data packet length */
    eth_msg_packt->flag = MR_NETDEV_FLAG_DNLD_DATA;
...
static int mr_eth_send_cmd_packet(struct mr_sdio_eth_priv *priv, uint8_t flag)
{
    ...
    skb_send = dev_alloc_skb(sizeof(struct mr_eth_msg_packt));
    if (!skb_send) {
        ETH_NETDEV_ERR(priv, "failed to allocate skb for cmd\n");
        return -ENOMEM;
    }

    eth_msg_packt = (struct mr_eth_msg_packt *)skb_put(skb_send, sizeof(struct mr_eth_msg_packt));
    eth_msg_packt->msg_packt.tag = priv->msg_tag;
    eth_msg_packt->msg_packt.sub_tag = 0;
    eth_msg_packt->msg_packt.len = sizeof(struct mr_eth_msg_packt) - sizeof(struct mr_msg_packt);
...
static int mr_eth_netdev_daemon_thread(void *data)
{
    ...
    while (1) {
        ...
        wait_event_interruptible(priv->waitq, !skb_queue_empty(&priv->thread_skb_head) || priv->thread_condition ||
                                                  kthread_should_stop() || priv->eth_status == MR_NETDEV_HSTA_RESET);
        ...
        if (skb_recv) {
            eth_msg_packt = (struct mr_eth_msg_packt *)skb_recv->data;
        } else {
            eth_msg_packt = NULL;
        }

        /* Anytime receiving DEVICE_RESET, need to return to initial state */
        if (eth_msg_packt && eth_msg_packt->flag == MR_NETDEV_FLAG_DEVICE_RESET) {
        ...
        switch (priv->eth_status) {
            ...
            case MR_NETDEV_HSTA_HOST_READY:
                ...
                if (eth_msg_packt == NULL || eth_msg_packt->flag != MR_NETDEV_FLAG_DEVICE_START) {
                    break;
                }
                ...
                if (eth_msg_packt->msg_packt.len <
                    6 + 4 + (sizeof(struct mr_eth_msg_packt) - sizeof(struct mr_msg_packt))) {
                ...
                eth_hw_addr_set(netdev, &eth_msg_packt->data[0]);
#else
                memcpy(netdev->dev_addr, &eth_msg_packt->data[0], ETH_ALEN);
#endif
                rtnl_unlock();
            ...
            case MR_NETDEV_HSTA_DEVICE_RUN:
                ...
                if (eth_msg_packt == NULL) {
                    break;
                } else if (eth_msg_packt->flag == MR_NETDEV_FLAG_UPLD_DATA) {
                    /* Received uplink data packet, parse flow control data, then submit to network device */
                    if (eth_msg_packt->credit_update_flag) {
                        mr_eth_credit_limit_update(netdev, eth_msg_packt->credit_limit_cnt);
                    }
                    mr_eth_upld_recv_net_data(netdev, skb_recv);
                    skb_recv = NULL; /* skb is now owned by netif_rx */
                    break;

                } else if (eth_msg_packt->flag == MR_NETDEV_FLAG_CREDIT_UPDATE) {
                ...
                } else if (eth_msg_packt->flag == MR_NETDEV_FLAG_MAC_IP_UPDATE) {
                    /* Handled at beginning of loop, should not reach here */
                    /* Check if packet length is sufficient to contain MAC address */
                    if (eth_msg_packt->msg_packt.len <
                        6 + 4 + (sizeof(struct mr_eth_msg_packt) - sizeof(struct mr_msg_packt))) {
                        ETH_NETDEV_ERR(priv, "device ready with invalid MAC/IP address length\n");
                    ...
                    memcpy(&ip_addr.s_addr, eth_msg_packt->data + 6, 4);
                    ...
                    ETH_NETDEV_INFO(priv, "Device set MAC: %pM, IP: %pI4\n", netdev->dev_addr, &ip_addr);
                    break;

                } else if (eth_msg_packt->flag == MR_NETDEV_FLAG_DEVICE_STOP) {
...
struct net_device *mr_sdio_eth_netdev_init(struct mr_msg_ctrl *msg_ctrl, uint8_t msg_tag)
{
    ...
    netdev->needed_headroom = ALIGN(ETH_HLEN + sizeof(struct mr_eth_msg_packt), L1_CACHE_BYTES);
