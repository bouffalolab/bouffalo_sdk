/**
 * @file mr_bootrom.c
 * @brief SDIO Bootrom Firmware Download Driver
 * @details This module implements the bootrom firmware download functionality
 *          for chipsets over SDIO interface. It handles
 ...
 */
static void mr_bootrom_buff_clear(mr_sdio_bootrom_t *sdio_boot)
{
    if (!sdio_boot) {
        return;
    }

    memset(sdio_boot->sdio_cmd_buff, 0, MR_BOOTROM_SDIO_CMD_BUFF_MAX);
    memset(sdio_boot->sdio_ack_buff, 0, MR_BOOTROM_SDIO_ACK_BUFF_MAX);
}
 ...
 */
static int mr_bootrom_send_cmd(struct mr_sdio_card *card, bootrom_cmd_t *cmd)
{
    int ret;
    ...
    struct sdio_trans_desc trans_desc = {
        ...
        .buff_len = MR_BOOTROM_SDIO_CMD_BUFF_MAX,
    };
    ret = card->sdio_dnld_transf(card, &trans_desc, MR_BOOTROM_SDIO_TRANS_TIMEOUT);
 ...
 */
static int mr_bootrom_receive_ack(struct mr_sdio_card *card, bootrom_ack_t *ack)
{
    int ret;
    ...
    struct sdio_trans_desc trans_desc = {
        .buff = (void *)ack,
        .data_len = 0,
        .buff_len = MR_BOOTROM_SDIO_ACK_BUFF_MAX,
    };
    ret = card->sdio_upld_transf(card, &trans_desc, MR_BOOTROM_SDIO_TRANS_TIMEOUT);
 ...
 */
static int mr_bootrom_get_firmware_image(struct mr_sdio_card *card, mr_sdio_bootrom_t *sdio_boot, char *fw_name)
{
    int ret;
    ...
    SDIO_DRV_INFO(card, "get firmware image OK, size: %d", (int)sdio_boot->mr_fw->size);
 ...
 */
static int mr_bootrom_get_boot_info(struct mr_sdio_card *card, mr_sdio_bootrom_t *sdio_boot)
{
    int ret;
    ...
    /* clear cmd/ack buff */
    mr_bootrom_buff_clear(sdio_boot);

    /* cmd fill */
    cmd->cmd = MR_BOOTROM_CMD_BOOTINFO_GET;
    cmd->sum = 0;
    cmd->len = 0;
    /* send cmd */
    ret = mr_bootrom_send_cmd(card, cmd);
    if (ret < 0) {
    ...
    /* receive ack */
    ret = mr_bootrom_receive_ack(card, ack);
    if (ret < 0) {
    ...
    /* bootinfo parse */
    if (ack->status != MR_BOOTOM_ACK_STA_OK) {
 ...
 */
static int mr_bootrom_download_bootheader(struct mr_sdio_card *card, mr_sdio_bootrom_t *sdio_boot)
{
    int ret;
    bootrom_cmd_t *cmd = (bootrom_cmd_t *)sdio_boot->sdio_cmd_buff;
    bootrom_ack_t *ack = (bootrom_ack_t *)sdio_boot->sdio_ack_buff;
    ...
    /* check image size */
    if (sdio_boot->mr_fw->size - sdio_boot->fw_curr_offset < sizeof(bootheader_t)) {
    ...
    /* cmd fill */
    cmd->cmd = MR_BOOTROM_CMD_BOOTHEADER_LOAD;
    cmd->sum = 0;
    cmd->len = sizeof(bootheader_t);
    memcpy(cmd->data, &(sdio_boot->mr_fw->data[sdio_boot->fw_curr_offset]), sizeof(bootheader_t));
    /* send cmd */
    ret = mr_bootrom_send_cmd(card, cmd);
    if (ret < 0) {
 ...
 */
static int mr_bootrom_download_sign_sha348_p2(struct mr_sdio_card *card, mr_sdio_bootrom_t *sdio_boot)
{
    int ret;
    bootrom_cmd_t *cmd = (bootrom_cmd_t *)sdio_boot->sdio_cmd_buff;
    bootrom_ack_t *ack = (bootrom_ack_t *)sdio_boot->sdio_ack_buff;
    ...
    /* Check image size */
    if (sdio_boot->mr_fw->size - sdio_boot->fw_curr_offset < 16) {
    ...
    /* Command fill */
    cmd->cmd = MR_BOOTROM_CMD_LOAD_BOOTHEADER_SHA384_P2;
    cmd->sum = 0;
    cmd->len = 16;
    memcpy(cmd->data, &(sdio_boot->mr_fw->data[sdio_boot->fw_curr_offset]), 16);
    /* Send cmd */
    ret = mr_bootrom_send_cmd(card, cmd);
    if (ret < 0) {
 ...
 */
static int mr_bootrom_download_sign_publickey1(struct mr_sdio_card *card, mr_sdio_bootrom_t *sdio_boot)
{
    int ret;
    ...
    /* check image size */
    if (sdio_boot->mr_fw->size - sdio_boot->fw_curr_offset < sign_publickey_size) {
    ...
    /* cmd fill */
    cmd->cmd = MR_BOOTROM_CMD_PK1_LOAD;
    cmd->sum = 0;
    cmd->len = sign_publickey_size;
    memcpy(cmd->data, &(sdio_boot->mr_fw->data[sdio_boot->fw_curr_offset]), sign_publickey_size);
    /* send cmd */
    ret = mr_bootrom_send_cmd(card, cmd);
    if (ret < 0) {
 ...
 */
static int mr_bootrom_download_sign_data1(struct mr_sdio_card *card, mr_sdio_bootrom_t *sdio_boot)
{
    int ret;
    int sign_data1_size;
    bootrom_cmd_t *cmd = (bootrom_cmd_t *)sdio_boot->sdio_cmd_buff;
    ...
    /* check image size */
    if (sdio_boot->mr_fw->size - sdio_boot->fw_curr_offset < sign_data1_size) {
    ...
    /* cmd fill */
    cmd->cmd = MR_BOOTROM_CMD_SIGNATURE1_LOAD;
    cmd->sum = 0;
    cmd->len = sign_data1_size;
    memcpy(cmd->data, &(sdio_boot->mr_fw->data[sdio_boot->fw_curr_offset]), sign_data1_size);
    /* send cmd */
    ret = mr_bootrom_send_cmd(card, cmd);
    if (ret < 0) {
 ...
 */
static int mr_bootrom_download_encry_iv(struct mr_sdio_card *card, mr_sdio_bootrom_t *sdio_boot)
{
    int ret;
    bootrom_cmd_t *cmd = (bootrom_cmd_t *)sdio_boot->sdio_cmd_buff;
    bootrom_ack_t *ack = (bootrom_ack_t *)sdio_boot->sdio_ack_buff;
    ...
    /* check image size */
    if (sdio_boot->mr_fw->size - sdio_boot->fw_curr_offset < sizeof(encry_iv_t)) {
    ...
    /* cmd fill */
    cmd->cmd = MR_BOOTROM_CMD_AESIV_LOAD;
    cmd->sum = 0;
    cmd->len = sizeof(encry_iv_t);
    memcpy(cmd->data, &(sdio_boot->mr_fw->data[sdio_boot->fw_curr_offset]), sizeof(encry_iv_t));
    /* send cmd */
    ret = mr_bootrom_send_cmd(card, cmd);
    if (ret < 0) {
 ...
 */
static int mr_bootrom_download_segment(struct mr_sdio_card *card, mr_sdio_bootrom_t *sdio_boot)
{
    int ret;
    ...
    if (sdio_boot->mr_fw->size - sdio_boot->fw_curr_offset < sizeof(segment_header_t)) {
    ...
    cmd->cmd = MR_BOOTROM_CMD_SECTIONHEADER_LOAD;
    cmd->sum = 0;
    cmd->len = sizeof(segment_header_t);
    memcpy(cmd->data, &(sdio_boot->mr_fw->data[sdio_boot->fw_curr_offset]), sizeof(segment_header_t));
    /* send cmd */
    ret = mr_bootrom_send_cmd(card, cmd);
    if (ret < 0) {
    ...
    SDIO_DRV_INFO(card, "download segment_header OK, dest_addr: 0x%08X, size: %d", segm_head_ack.dest_addr,
                  segm_head_ack.len);
    /* update offset */
    sdio_boot->fw_curr_offset += sizeof(segment_header_t);

    /* check segment_data size */
    if (sdio_boot->mr_fw->size - sdio_boot->fw_curr_offset < segm_head_ack.len) {
    ...
    for (send_cnt = 0, send_len = 0; send_len < segm_head_ack.len; send_cnt++) {
        ...
        cmd->cmd = MR_BOOTROM_CMD_SECTIONDATA_LOAD;
        cmd->sum = 0;
        if (segm_head_ack.len - send_len > MR_BOOTROM_CMD_PAYLOAD_MAX) {
            cmd->len = MR_BOOTROM_CMD_PAYLOAD_MAX;
        } else {
            cmd->len = segm_head_ack.len - send_len;
        }
        memcpy(cmd->data, &(sdio_boot->mr_fw->data[sdio_boot->fw_curr_offset]), cmd->len);
        /* send cmd */
        ret = mr_bootrom_send_cmd(card, cmd);
        if (ret < 0) {
        ...
        /* check ack */
        if (ack->status != MR_BOOTOM_ACK_STA_OK) {
            SDIO_DRV_ERR(card, "segment_data piece[%d] ack status err: 0x%04X, len: 0x%04X", send_cnt, ack->status,
                         ack->len);
            return -1;
        }
        ...
        SDIO_DRV_INFO(card, "download segment_data piece[%d] OK, size:%d, progress:%d/%d", send_cnt, cmd->len, send_len,
                      segm_head_ack.len);
    }
 ...
 */
static int mr_bootrom_check_image(struct mr_sdio_card *card, mr_sdio_bootrom_t *sdio_boot)
{
    int ret;
    bootrom_cmd_t *cmd = (bootrom_cmd_t *)sdio_boot->sdio_cmd_buff;
    bootrom_ack_t *ack = (bootrom_ack_t *)sdio_boot->sdio_ack_buff;
    ...
    /* cmd fill */
    cmd->cmd = MR_BOOTROM_CMD_CHECK_IMAGE;
    cmd->sum = 0;
    cmd->len = 0;
    /* send cmd */
    ret = mr_bootrom_send_cmd(card, cmd);
 ...
 */
int mr_bootrom_download_firmware(struct mr_sdio_card *card, char *fw_name)
{
    int ret = 0;
    int segment_num;
    mr_sdio_bootrom_t *sdio_boot;

    sdio_boot = kzalloc(sizeof(mr_sdio_bootrom_t), GFP_KERNEL);
    if (!sdio_boot) {
        ret = -ENOMEM;
        goto exit;
    }
    ...
    if (!fw_name) {
        fw_name = MR_BOOTROM_FW_NAME;
    }

    /* 1. get firmware image */
    SDIO_DRV_INFO(card, "request_firmware %s", fw_name);
    ret = mr_bootrom_get_firmware_image(card, sdio_boot, fw_name);
    if (ret < 0) {
    ...
    ret = mr_bootrom_get_boot_info(card, sdio_boot);
    if (ret < 0) {
    ...
    ret = mr_bootrom_download_bootheader(card, sdio_boot);
    if (ret < 0) {
    ...
    if (sdio_boot->chip_sign_type && sdio_boot->fw_sign_type) {
        ...
        if (sdio_boot->chip_sign_type == 2) {
            ret = mr_bootrom_download_sign_sha348_p2(card, sdio_boot);
            if (ret < 0) {
        ...
        ret = mr_bootrom_download_sign_publickey1(card, sdio_boot);
        if (ret < 0) {
        ...
        ret = mr_bootrom_download_sign_data1(card, sdio_boot);
        if (ret < 0) {
    ...
    if (sdio_boot->chip_encry_type && sdio_boot->fw_encry_type) {
        ...
        ret = mr_bootrom_download_encry_iv(card, sdio_boot);
        if (ret < 0) {
    ...
    for (segment_num = 0; segment_num < sdio_boot->fw_segment_num; segment_num++) {
        ...
        ret = mr_bootrom_download_segment(card, sdio_boot);
        if (ret < 0) {
    ...
    ret = mr_bootrom_check_image(card, sdio_boot);
    if (ret < 0) {
...
exit:
    if (sdio_boot->mr_fw) {
        release_firmware(sdio_boot->mr_fw);
    }
    if (sdio_boot) {
        kfree(sdio_boot);
    }
 ...
 */
int mr_bootrom_firmware_run(struct mr_sdio_card *card)
{
    int ret = 0;
    mr_sdio_bootrom_t *sdio_boot;
    bootrom_cmd_t *cmd;
