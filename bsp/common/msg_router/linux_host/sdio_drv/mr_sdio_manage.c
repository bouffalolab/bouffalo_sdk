 ...
 */
static void mr_sdio_lock(struct mr_sdio_manage *sdio_manage)
{
    if (sdio_manage) {
 ...
 * @param[in] sdio_manage Pointer to SDIO management structure
 */
static void mr_sdio_unlock(struct mr_sdio_manage *sdio_manage)
{
    if (sdio_manage) {
 ...
 */
static int mr_sdio_dnld_polling(struct mr_sdio_manage *sdio_manage)
{
    int ret;
    struct sk_buff *skb;
 ...
 */
static int mr_sdio_upld_polling(struct mr_sdio_manage *sdio_manage)
{
    int ret;
    struct sk_buff *skb;
 ...
 */
static void mr_sdio_workqueue_func(struct work_struct *work)
{
    struct mr_sdio_manage *sdio_manage = container_of(work, struct mr_sdio_manage, sdio_work);
    int processed = 0;
    ...
    while (1) {
        ...
        if (upld_proc_flag) {
            upld_proc_flag = mr_sdio_upld_polling(sdio_manage);
        }
        ...
        processed += 1;
        if (processed >= 256) {
            SDIO_DRV_DBG(sdio_manage->sdio_card, "Processed %d packets in this workqueue run, dnld:%d, upld:%d",
                         processed, dnld_proc_flag, upld_proc_flag);
            if (upld_proc_flag) {
 ...
 */
static void mr_sdio_upld_irq_cb(struct mr_sdio_manage *sdio_manage)
{
    if (!sdio_manage) {
        return;
    }
 ...
 */
int mr_sdio_manage_dnld_send(struct mr_sdio_manage *sdio_manage, struct sk_buff *skb)
{
 ...
 */
int mr_sdio_manage_upld_recv(struct mr_sdio_manage *sdio_manage, struct sk_buff **skb)
{
 ...
 */
int mr_sdio_manage_deinit(struct mr_sdio_manage *sdio_manage)
{
    if (!sdio_manage) {
        return -1;
    }
    ...
    SDIO_DRV_INFO(sdio_manage->sdio_card, "mr_sdio_manage_deinit");
 ...
 * @param[out] mr_sdio_manage Pointer to receive allocated SDIO management structure
 ...
 */
int mr_sdio_manage_init(struct mr_sdio_manage **mr_sdio_manage, struct mr_sdio_card *card)
{
    int ret = 0;
    struct mr_sdio_manage *sdio_manage = NULL;

    /* Allocate SDIO management structure */
    sdio_manage = kzalloc(sizeof(struct mr_sdio_manage), GFP_KERNEL);
    if (!sdio_manage) {
        ret = -ENOMEM;
        goto err_exit;
    }
    ...
    sdio_manage->sdio_workqueue = alloc_workqueue("MR_SDIO_MANAGE_WQ", WQ_HIGHPRI | WQ_UNBOUND | WQ_MEM_RECLAIM, 1);
    if (!sdio_manage->sdio_workqueue) {
        SDIO_DRV_ERR(card, "creat dnld workqueue failed");
        ret = -EINVAL;
        goto err_exit;
    }
    INIT_WORK(&sdio_manage->sdio_work, mr_sdio_workqueue_func);

    /* Enable card-to-host interrupt */
    ret = card->sdio_upld_irq_register(card, mr_sdio_upld_irq_cb);
    if (ret < 0) {
    ...
    if (mr_sdio_manage) {
        *mr_sdio_manage = sdio_manage;
    }

    /* first trigger */
    mr_sdio_upld_irq_cb(sdio_manage);
 ...
 */
void mr_sdio_manage_cplt_cb_register(struct mr_sdio_manage *sdio_manage, trans_cplt_cb upld_recv_cb,
                                     void *upld_recv_arg, trans_cplt_cb dnld_send_cb, void *dnld_send_arg)
{
    if (!sdio_manage) {
        return;
    }
...
extern struct sdio_driver mr_sdio2;
extern struct sdio_driver mr_sdio3;
 ...
 */
static int mr_sdio_module_init(void)
{
    int ret;
    printk("MR sdio module init\n\n");

    printk("register MR sdio2 driver\n");
    ret = sdio_register_driver(&mr_sdio2);
    if (ret < 0) {
        pr_err("register sdio2 failed\n");
        return ret;
    }

    printk("register MR sdio3 driver\n");
    ret = sdio_register_driver(&mr_sdio3);
    if (ret < 0) {
 ...
 */
static void mr_sdio_module_exit(void)
{
    printk("unregister MR sdio2 driver\n");
    sdio_unregister_driver(&mr_sdio2);

    printk("unregister MR sdio3 driver\n");
    sdio_unregister_driver(&mr_sdio3);

    printk("MR sdio module exit\n\n");
}

module_init(mr_sdio_module_init);
module_exit(mr_sdio_module_exit);
MODULE_LICENSE("GPL");
