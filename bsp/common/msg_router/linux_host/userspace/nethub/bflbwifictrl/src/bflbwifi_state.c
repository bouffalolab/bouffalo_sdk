...
/* GOTIP 参数解析函数（仅在启用自动配置时需要） */
extern int bflbwifi_parser_parse_gotip(const char *line, char *ip, char *gw, char *mask, char *dns);

/* 网卡配置函数（仅在启用自动配置时需要） */
extern int bflbwifi_netif_config(const char *ifname,
                                  const char *ip,
                                  const char *mask,
                                  const char *gw,
                                  const char *dns);
 ...
 * @param line URC line (may contain IP parameters)
 */
static void handle_wifi_gotip(const char *line)
{
#ifdef BFLBWIFI_NETIF_AUTO_CONFIG
    char ip[16] = {0};
    char gw[16] = {0};
    ...
    /* 解析 GOTIP 参数 */
    if (line && bflbwifi_parser_parse_gotip(line, ip, gw, mask, dns) == 0) {
        /* 检查是否成功解析到参数 */
        if (ip[0] != '\0' && gw[0] != '\0' && mask[0] != '\0' && dns[0] != '\0') {
            /* 获取当前连接信息 */
            pthread_mutex_lock(&g_state.mutex);
            ...
            strncpy(info.gateway, gw, sizeof(info.gateway) - 1);
            info.gateway[sizeof(info.gateway) - 1] = '\0';
            strncpy(info.netmask, mask, sizeof(info.netmask) - 1);
            info.netmask[sizeof(info.netmask) - 1] = '\0';
            strncpy(info.dns, dns, sizeof(info.dns) - 1);
            info.dns[sizeof(info.dns) - 1] = '\0';

            /* 更新状态信息 */
            bflbwifi_state_set_sta_info(&info);

            /* 配置网卡 */
            if (bflbwifi_netif_config("mr_eth0", ip, mask, gw, dns) == 0) {
                BFLB_LOGI("WiFi: GOTIP (IP:%s, GW:%s, MASK:%s, DNS:%s) - 网卡已配置",
                          ip, gw, mask, dns);
            } else {
                BFLB_LOGE("WiFi: GOTIP (IP:%s, GW:%s, MASK:%s, DNS:%s) - 网卡配置失败",
                          ip, gw, mask, dns);
            }
        } else {
            BFLB_LOGI("WiFi: GOTIP (无参数)");
        }
    } else {
