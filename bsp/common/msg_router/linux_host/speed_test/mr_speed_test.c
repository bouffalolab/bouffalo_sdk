 ...
 * @brief SDIO Speed Test Implementation
 ...
 */
static int mr_speed_test_buff_uninit(struct mr_speed_test *speed_test)
{
    int32_t i;
 ...
 */
static int mr_speed_test_buff_init(struct mr_speed_test *speed_test)
{
    int32_t i, j;
    uint8_t *buff;
...
static int mr_speed_test_dnld(struct mr_speed_test *speed_test, uint8_t *dnld_test_buff)
{
    ...
    if (ret < 0) {
        SPEED_TEST_ERR(speed_test, "mr_speed_test_dnld send error %d", ret);
 ...
 */
static int mr_speed_test_dnld_cb(struct sk_buff *skb, bool success, void *arg)
{
...
static int mr_speed_test_upld_cb(struct sk_buff *skb, bool success, void *arg)
{
    ...
    if (speed_test->upld_cnt >= MR_SPEED_TEST_BUFF_CNT) {
 ...
 */
static int mr_speed_test_transfer_wait(struct mr_speed_test *speed_test, uint32_t expected_cnt, int timeout_ms)
{
    ...
    while (1) {
        ...
        if (current_dnld_cnt >= expected_cnt && current_upld_cnt >= expected_cnt) {
            SPEED_TEST_DBG(speed_test, "transfer_wait completed: dnld=%d, upld=%d", current_dnld_cnt, current_upld_cnt);
            break;
        }

        if (time_after(jiffies, timeout)) {
            SPEED_TEST_DBG(speed_test, "transfer_wait timeout: dnld=%d/%d, upld=%d/%d!\n", current_dnld_cnt,
                           expected_cnt, current_upld_cnt, expected_cnt);
            return -ETIMEDOUT;
        }
 ...
 */
static int mr_speed_test_check(struct mr_speed_test *speed_test)
{
    int i, j;
    int check_size;
    ...
    for (i = 0; i < MR_SPEED_TEST_BUFF_CNT; i++) {
        ...
        for (j = 0; j < check_size; j++) {
            if (upld_buff[j] != dnld_buff[j]) {
                SPEED_TEST_ERR(speed_test, "data mismatch in packet %d at byte %d: %02X->%02X", i, j, dnld_buff[j],
                               upld_buff[j]);
                error_count++;
 ...
 */
static int mr_speed_test(struct mr_speed_test *speed_test)
{
    int ret = 0, i = 0, j = 0;
    ...
    ret = mr_speed_test_buff_init(speed_test);
    if (ret < 0) {
    ...
    ret = mr_msg_cb_register(speed_test->msg_ctrl, speed_test->msg_tag, mr_speed_test_upld_cb, speed_test,
                             mr_speed_test_dnld_cb, speed_test);
    if (ret < 0) {
        SPEED_TEST_ERR(speed_test, "mr_msg_cb_register error %d!\n", ret);
        goto return_out;
    }

    if (speed_test->thread_mode) {
        mr_speed_test_transfer_wait(speed_test, MR_SPEED_TEST_BUFF_CNT, MR_SPEED_TEST_BUFF_CNT);
        spin_lock(&speed_test->stats_lock);
        speed_test->dnld_cnt = 0;
        speed_test->upld_cnt = 0;
        spin_unlock(&speed_test->stats_lock);
        /* try: */
        ret = mr_speed_test_dnld(speed_test, speed_test->dnld_test_buff[0]);
        if (ret < 0) {
            goto try_exit;
        }
        ret = mr_speed_test_transfer_wait(speed_test, 1, 100);
        if (ret < 0) {
            goto try_exit;
        }
    }

    SPEED_TEST_INFO(speed_test, "test cnt: %d*%d, speedtest start ...\n", MR_SPEED_TEST_BUFF_CNT, MR_SPEED_TEST_CNT);
    ...
    for (i = 0; i < MR_SPEED_TEST_CNT; i++) {
        ...
        for (j = 0; j < MR_SPEED_TEST_BUFF_CNT; j++) {
            ret = mr_speed_test_dnld(speed_test, speed_test->dnld_test_buff[j]);
            if (ret < 0) {
                goto return_out;
            }
        }
        ...
        ret = mr_speed_test_transfer_wait(speed_test, MR_SPEED_TEST_BUFF_CNT, MR_SPEED_TEST_BUFF_CNT * 20);
        if (ret < 0) {
            goto return_out;
        }
...
return_out:
    ...
    if (ret < 0) {
        SPEED_TEST_INFO(speed_test, "mr_mr_speedtest fail: %d/%d!!! \n", i, MR_SPEED_TEST_CNT);
    } else {
        SPEED_TEST_INFO(speed_test, "mr_mr_speedtest success: %d/%d!!! \n", i, MR_SPEED_TEST_CNT);
    }

try_exit:

    /* Unregister message receive callback */
    mr_msg_cb_unregister(speed_test->msg_ctrl, speed_test->msg_tag);

    /* Release test buffers */
    mr_speed_test_buff_uninit(speed_test);
...
static struct mr_speed_test *g_speed_test_ptr[MR_MSG_TAG_MAX] = { NULL };
 ...
 *          testing scenarios.
 */
static int mr_speed_test_thread(void *data)
{
    int ret;
    int delay;
    struct mr_speed_test *speed_test = (struct mr_speed_test *)data;
    ...
    SPEED_TEST_INFO(speed_test, "mr_speed_test_thread run\n");

    while (!kthread_should_stop()) {
        ret = mr_speed_test(speed_test);
    ...
    SPEED_TEST_INFO(speed_test, "mr_speed_test_thread stop\n\n");
 ...
 */
int mr_speed_test_run(struct mr_msg_ctrl *msg_ctrl, uint8_t msg_tag, bool thread_mode)
{
    int ret;
    struct mr_speed_test *speed_test;

    if (msg_tag >= MR_MSG_TAG_MAX) {
        pr_err("[mr_speed_test %d] mr_speed_test_run invalid msg_tag\n", msg_tag);
        return -EINVAL;
    }

    if (thread_mode && g_speed_test_ptr[msg_tag]) {
        pr_notice("[mr_speed_test %d] mr_speed_test_run task already exists\n", msg_tag);
        return -EINVAL;
    }

    speed_test = kzalloc(sizeof(struct mr_speed_test), GFP_KERNEL);
    if (!speed_test) {
        return -ENOMEM;
    }
    ...
    SPEED_TEST_INFO(speed_test, "mr_speed_test_thread create: %d\n", msg_tag);

    speed_test->thread = kthread_create(mr_speed_test_thread, speed_test, "mr_speed_test_%d", msg_tag);
    if (IS_ERR(speed_test->thread)) {
        SPEED_TEST_INFO(speed_test, "mr_speed_test_thread create failed!\n");
        kfree(speed_test);
    ...
    SPEED_TEST_INFO(speed_test, "mr_speed_test_thread_create success!\n");
    return 0;
}
 ...
 */
int mr_speed_test_thread_destroy(uint8_t msg_tag)
{
    if (msg_tag >= MR_MSG_TAG_MAX) {
        pr_notice("[mr_speed_test %d] mr_speed_test_thread_destroy invalid msg_tag\n", msg_tag);
        return -EINVAL;
    }

    if (!g_speed_test_ptr[msg_tag]) {
        pr_notice("[mr_speed_test %d] mr_speed_test_thread_destroy no task\n", msg_tag);
        return -ENOENT;
    }
    ...
    pr_notice("[mr_speed_test %d] mr_speed_test_thread_destroy success \n", msg_tag);
    return 0;
}
 ...
 */
int mr_speed_test_thread_destroy_all(void)
{
    int i;

    for (i = 0; i < MR_MSG_TAG_MAX; i++) {
        if (g_speed_test_ptr[i]) {
            mr_speed_test_thread_destroy(i);
        }
    }
