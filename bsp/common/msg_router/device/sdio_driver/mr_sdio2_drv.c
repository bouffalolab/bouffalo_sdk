 ...
 * @retval None
 */
static void sdio2_dnld_irq_callback(void *arg, bflb_sdio2_trans_desc_t *trans_desc, bool success)
{
 ...
 * @retval None
 */
static void sdio2_upld_irq_callback(void *arg, bflb_sdio2_trans_desc_t *trans_desc, bool success)
{
    if (arg == NULL || trans_desc == NULL) {
        return;
    }
...
static void sdio2_irq_cb(void *arg, uint32_t irq_event, bflb_sdio2_trans_desc_t *trans_desc)
{
    switch (irq_event) {
        case SDIO2_IRQ_EVENT_DNLD_CPL:
            sdio2_dnld_irq_callback(arg, trans_desc, true);
            break;

        case SDIO2_IRQ_EVENT_UPLD_CPL:
            sdio2_upld_irq_callback(arg, trans_desc, true);
            break;
        ...
        case SDIO2_IRQ_EVENT_ERR_CRC:
            ...
            sdio2_dnld_irq_callback(arg, trans_desc, false);
            break;
 ...
 * @retval -1 Error
 */
static int sdio2_driver_init(mr_msg_ctrl_priv_t *msg_ctrl)
{
 ...
 */
static int sdio2_is_ready(void)
{
 ...
 */
static int sdio2_dnld_is_busy(void)
{
 ...
 * @retval -1 Error
 */
static int sdio2_dnld_start(mr_frame_elem_t *frame_elem)
{
    int ret;
 ...
 */
static int sdio2_upld_is_busy(void)
{
    if (sdio2_ready_flag == false) {
        LOG_E("sdio2 not ready\r\n");
        return -1;
 ...
 * @retval -1 Error
 */
static int sdio2_upld_start(mr_frame_elem_t *frame_elem)
{
    int ret;
    uint16_t blk_size;
...
mr_msg_ctrl_ops_t msg_ctrl_sdio_ops = {
    .name = "SDIO2",
    .dev_init = sdio2_driver_init,
    .dev_deinit = NULL,
    .dev_is_ready = sdio2_is_ready,
    .dev_dnld_is_busy = sdio2_dnld_is_busy,
    .dev_upld_is_busy = sdio2_upld_is_busy,
    .dev_dnld_start = sdio2_dnld_start,
    .dev_upld_start = sdio2_upld_start,
};