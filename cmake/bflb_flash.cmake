...
foreach(item ${CONFIG_POST_BUILDS})
    ...
    elseif("${item}" STREQUAL "GENERATE_ROMFS")
        list(APPEND post_build_cmds 
            ...
            COMMAND ${BL_SDK_BASE}/tools/genromfs/genromfs${TOOL_SUFFIX} -d romfs/ -f ./build/build_out/romfs.bin)

    elseif("${item}" STREQUAL "GENERATE_LITTLEFS")
        list(APPEND post_build_cmds 
            COMMAND ${CMAKE} -E echo "[littlefs] generate littlefs.bin using littlefs directory"
            COMMAND ${BL_SDK_BASE}/tools/genlfs/mklfs${TOOL_SUFFIX} -c lfs -b 4096 -p 256 -r 256 -s 0x71000 -i ./build/build_out/littlefs.bin)
    endif()
endforeach()
...
if(BUILD_BINS)
    file(GLOB BUILD_BINS 
        "${CMAKE_CURRENT_SOURCE_DIR}/build/build_out/*.bin" 
        "${CMAKE_CURRENT_SOURCE_DIR}/build/build_out/*.elf" 
        "${CMAKE_CURRENT_SOURCE_DIR}/build/build_out/*.xz" 
        "${CMAKE_CURRENT_SOURCE_DIR}/build/build_out/*.ota")
