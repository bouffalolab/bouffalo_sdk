...
static void set_cpu_bclk_80M_and_gate_clk(void)
{
    ...
    CPU_Set_MTimer_CLK(ENABLE, BFLB_MTIMER_SOURCE_CLOCK_MCU_CLK, Clock_System_Clock_Get(BFLB_SYSTEM_CLOCK_MCU_CLK) / 1000000 - 1);

    /* clk gate,except DMA&CPU&UART0&SF&EMI&WIFI&EFUSE */
    tmpVal = 0;
    tmpVal = BFLB_SET_REG_BITS_VAL(tmpVal, GLB_CGEN_M_CPU, 1);
    tmpVal = BFLB_SET_REG_BITS_VAL(tmpVal, GLB_CGEN_M_DMA, 1);
    tmpVal = BFLB_SET_REG_BITS_VAL(tmpVal, GLB_CGEN_M_SEC, 1);
    tmpVal = BFLB_SET_REG_BITS_VAL(tmpVal, GLB_CGEN_M_SDU, 1);
    BFLB_WR_REG(GLB_BASE, GLB_CGEN_CFG0,tmpVal);

    tmpVal = 0;
    tmpVal = BFLB_SET_REG_BITS_VAL(tmpVal, GLB_CGEN_S1_EF_CTRL, 1);
    tmpVal = BFLB_SET_REG_BITS_VAL(tmpVal, GLB_CGEN_S1_SF_CTRL, 1);
    tmpVal = BFLB_SET_REG_BITS_VAL(tmpVal, GLB_CGEN_S1_DMA, 1);
    tmpVal = BFLB_SET_REG_BITS_VAL(tmpVal, GLB_CGEN_S1A_UART0, 1);
    tmpVal = BFLB_SET_REG_BITS_VAL(tmpVal, GLB_CGEN_S1A_UART1, 1);
    tmpVal = BFLB_SET_REG_BITS_VAL(tmpVal, GLB_CGEN_S1_SEC_ENG, 1);
    BFLB_WR_REG(GLB_BASE, GLB_CGEN_CFG1,tmpVal);

    tmpVal = 0;
    tmpVal = BFLB_SET_REG_BITS_VAL(tmpVal, GLB_CGEN_S2_WIFI, 1);
    tmpVal = BFLB_SET_REG_BITS_VAL(tmpVal, GLB_CGEN_S1_EXT_EMI_MISC, 1);
    tmpVal = BFLB_SET_REG_BITS_VAL(tmpVal, GLB_CGEN_S1_EXT_PIO, 1);

    BFLB_WR_REG(GLB_BASE, GLB_CGEN_CFG2, tmpVal);
}
...
extern bflb_lp_fw_cfg_t lpfw_cfg;
...
static int test_tcp_keepalive(int argc, char **argv)
{
    ...
    while (1) {
        ...
        if (pck_cnt && (pck_total >= pck_cnt)) {
            bflb_pm_event_bit_set(PSM_EVENT_APP);
            break;
        }
#if 0
...
void app_pm_enter_hbn(uint32_t ms, int level)
{
    ...
    bflb_lp_hbn_fw_cfg_t hbn_test_cfg={
