...
struct bt_conn *bflb_blesync_conn = NULL;
struct bt_gatt_exchange_params bflb_blesync_exchg_mtu;
...
static void bflb_blesync_exchange_func(struct bt_conn *conn, u8_t err,
			  struct bt_gatt_exchange_params *params)
{
	printf("Exchange %s\r\n", err == 0U ? "successful" : "failed");
}

static void bflb_blesync_disconnected(struct bt_conn *conn, u8_t reason)
{
	...
	bflb_blesync_conn = NULL;
}

static void bflb_blesync_connected(struct bt_conn *conn, u8_t err)
{
	...
	printf("%s\r\n",__func__);
	bflb_blesync_conn = conn;

	//exchange mtu size after connected.
	bflb_blesync_exchg_mtu.func = bflb_blesync_exchange_func;
	ret = bt_gatt_exchange_mtu(bflb_blesync_conn, &bflb_blesync_exchg_mtu);
	if (!ret) {
        ...
        switch (p_event->event_id) {
          ...
          case CMD_GET_VERSION:
              pro_trans_layer_ack_read(gp_index->pro_handle,
                                       BFLB_BLESYNC_BLE_VERSION,
                                       sizeof(BFLB_BLESYNC_BLE_VERSION));
              break;
#if defined(CONFIG_ZIGBEE_PROV)
          case CMD_ZB_SCAN:
...
static struct bt_gatt_service wifiprov_server = BT_GATT_SERVICE(attrs);

static struct bt_conn_cb bflb_blesync_conn_callbacks = {
	.connected	=   bflb_blesync_connected,
	.disconnected	=   bflb_blesync_disconnected,
};
                       ...
                       void *cb_arg)
{
    ...
    {
        isRegister = 1;
        bt_conn_cb_register(&bflb_blesync_conn_callbacks);
    }
