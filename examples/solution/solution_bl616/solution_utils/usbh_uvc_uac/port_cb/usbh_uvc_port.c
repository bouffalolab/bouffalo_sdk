...
ATTR_FAST_RAM_SECTION void usbh_video_transfer_abort_callback(void)
{
    if (usbh_uvc_run_flag == false) {
        UVC_ERR("uvc is not running\r\n");
        return;
    }

    if (usbh_reset_flag) {
        UVC_ERR("usbh reset repeat\r\n");
        return;
    }
    usbh_reset_flag = true;

    UVC_ERR("usbh reset start\r\n");
...
static void usbh_uvc_check_cb(void *argument)
{
    ...
    if (uvc_check_frame_cnt == 0xffffffff) {
        uvc_check_frame_cnt = g_uvc_total_frame_cnt;
        return;
    }

    if (g_uvc_total_frame_cnt == uvc_check_frame_cnt) {
        UVC_ERR("uvc no frame, trigger reset\r\n");
...
/*  */
void usbh_video_auto_reset_init(void)
{
    usbh_reset_flag = false;
...
void usbh_video_auto_reset_deinit(void)
{
