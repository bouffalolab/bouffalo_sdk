...
static uint16_t jpeg_out_queue_rtc_id = MJPEG_OUT_FRAME_STREAM_WIFI_RTC_ID;

static jpeg_frame_t jpeg_frame_info[HB_TX_DEFAULT_FRAME_DEPTH] = { 0 };
...
static void hb_sender_loop_cb(void *arg)
{
    ...
    ret = frame_queue_output_pop(g_mjpeg_out_frame_ctrl, (frame_elem_t *)&jpeg_frame, jpeg_out_queue_rtc_id, 0);
    if (ret < 0) {
        return;
    }
...
int hb_sender_init(uint16_t local_port)
{
    ...
    if (sender_hd == NULL) {
        frame_queue_output_remove(g_mjpeg_out_frame_ctrl, jpeg_out_queue_rtc_id);
