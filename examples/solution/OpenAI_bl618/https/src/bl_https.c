...
static int proc_http_head(const uint8_t *buf, int buf_len, int *status_code, int *body_start_off, uint32_t *Content_Length)
{
    ...
    *status_code = atoi(status_code_buf);
    if ((tmp_buf = memmem(buf, buf_len, "Content-Length: ", 16)) != NULL){
        *Content_Length = atoi(tmp_buf+16);
        printf("======length:%s\r\n", tmp_buf);
    }
    printf("\r\nres:%s\r\n", buf);
            ...
            while(1) {
                ...
                if (rcv_ret > 0) {
                    ...
                    if (!http_head_proced) {
                        ...
                        int proc_head_r = proc_http_head(rcv_buf, rcv_ret, &status_code, &body_start_off, &Content_Length);
                        LOG_I("proc_head_r %d, status_code %d, body_start_off %d, Content_Length:%d\r\n", proc_head_r, status_code, body_start_off, Content_Length);
                    ...
                    //printf("http_head_proced:%d, resp_write_off:%d, Content_Length:%d\r\n", http_head_proced, resp_write_off, Content_Length);
                    if (http_head_proced && (resp_write_off == Content_Length)) {
                        LOG_I("read end\r\n");
                        break;
                    }
                    static int i=0;
