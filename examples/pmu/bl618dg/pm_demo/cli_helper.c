...
static const struct board_kys_v2_gpio_config_s *g_keyscan_config;

static int pm_demo_cli_gpio_conflicts_with_keyscan(uint8_t pin)
{
#if defined(CONFIG_EXAMPLE_PM_KEYSCAN_WAKEUP)
    uint8_t i;

    if (g_keyscan_config == NULL) {
        return 0;
    }
    for (i = 0; i < g_keyscan_config->row_count; i++) {
        if (g_keyscan_config->row_pins[i] == pin) {
            return 1;
        }
    }
    for (i = 0; i < g_keyscan_config->col_count; i++) {
        if (g_keyscan_config->col_pins[i] == pin) {
            return 1;
        }
    }
#else
    ...
    const struct board_lp_gpio_wakeup_config_s *wakeup)
{
    ...
    uint64_t valid_gpio_mask = (1ULL << GPIO_PIN_MAX) - 1ULL;
    uint64_t wakeup_mask = wakeup->io_wakeup_unmask;
    uint8_t pin;

    if ((((wakeup->io_ie | wakeup->io_pu | wakeup->io_pd | wakeup_mask) &
          ~valid_gpio_mask) != 0U) ||
        ((wakeup_mask & ~wakeup->io_ie) != 0U) ||
        ((wakeup->io_pu & wakeup->io_pd) != 0U)) {
        return -EINVAL;
    }

    for (pin = 0; pin < GPIO_PIN_MAX; pin++) {
        if (((wakeup_mask & (1ULL << pin)) != 0U) &&
            (pm_demo_cli_gpio_conflicts_with_keyscan(pin) != 0)) {
            printf("GPIO%u conflicts with the keyscan matrix\r\n", pin);
            return -EINVAL;
        }
    }
    ...
    gpio_cfg.io_wakeup_unmask = wakeup_mask;
    gpio_cfg.io_0_7_trig_mode = wakeup->io_0_7_trig_mode;
    gpio_cfg.io_8_15_trig_mode = wakeup->io_8_15_trig_mode;
    gpio_cfg.io_16_23_trig_mode = wakeup->io_16_23_trig_mode;
    gpio_cfg.io_24_31_trig_mode = wakeup->io_24_31_trig_mode;
    gpio_cfg.io_32_39_trig_mode = wakeup->io_32_39_trig_mode;
    gpio_cfg.io_40_47_trig_mode = wakeup->io_40_47_trig_mode;
    gpio_cfg.io_48_52_trig_mode = wakeup->io_48_52_trig_mode;

    return pm_lowpower_gpio_cfg(&gpio_cfg);
}

static uint8_t *pm_demo_cli_gpio_group_trigger(
    struct board_lp_gpio_wakeup_config_s *wakeup, uint8_t pin)
{
    if (pin <= 7U) {
        return &wakeup->io_0_7_trig_mode;
    }
    if (pin <= 15U) {
        return &wakeup->io_8_15_trig_mode;
    }
    if (pin <= 23U) {
        return &wakeup->io_16_23_trig_mode;
    }
    if (pin <= 31U) {
        return &wakeup->io_24_31_trig_mode;
    }
    if (pin <= 39U) {
        return &wakeup->io_32_39_trig_mode;
    }
    if (pin <= 47U) {
        return &wakeup->io_40_47_trig_mode;
    }
    return &wakeup->io_48_52_trig_mode;
}
    ...
    const struct board_kys_v2_gpio_config_s *keyscan_config)
{
    ...
    g_keyscan_config = keyscan_config;
    return 0;
}
...
static int app_pm_gpio(int argc, char **argv)
{
    ...
    pin_mask = 1ULL << pin;
    group_mask = (0xffULL << (pin & ~7U)) &
                 ((1ULL << GPIO_PIN_MAX) - 1ULL);
    group_trigger = pm_demo_cli_gpio_group_trigger(&candidate, pin);
    if (((candidate.io_wakeup_unmask & group_mask & ~pin_mask) != 0U) &&
        (*group_trigger != trigger)) {
        printf("GPIO%lu trigger conflicts with another enabled GPIO in "
               "group %lu-%lu\r\n",
               (unsigned long)pin, (unsigned long)(pin & ~7U),
               (unsigned long)(((pin & ~7U) + 7U) < GPIO_PIN_MAX ?
                                   ((pin & ~7U) + 7U) :
                                   (GPIO_PIN_MAX - 1U)));
        return -EINVAL;
    }
...
{
    return AON_Set_BOD_Config(1, threshold, 1);
}
...
static int app_pm_bod(int argc, char **argv)
{
    ...
    printf("BOD threshold=%lu (~%u.%02u V), output=%u\r\n",
           (unsigned long)threshold, threshold_mv / 1000U,
           (threshold_mv % 1000U) / 10U,
           (unsigned int)AON_Get_BOD_OUT_State());
    return 0;
}
