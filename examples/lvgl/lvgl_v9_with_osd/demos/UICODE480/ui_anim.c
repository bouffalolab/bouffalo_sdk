...
void ui_anim_start(void)
{
    ...
    /* Sweep both sliders min->max->min, forever, via a low-rate timer. Start
     * them at different phases so they don't move in lockstep. */
    ...
    slider_sweep_start(&ui_Slider2, 0);
}
