...
int main(void)
{
    ...
    /* Capture from builtin AUADC, play back through builtin AUDAC/AUPWM. */
    wave_player_hw_cfg_t audio_hw_cfg = {
        .dma_tx_dev_name = "dma0_ch0",
        .dma_rx_dev_name = "dma0_ch1",
        .audac_dev_name = "audac",
        .auadc_dev_name = "auadc",
        .playback_backend = WAVE_PLAYER_BACKEND_BL_CODEC,
        .capture_backend = WAVE_PLAYER_BACKEND_BL_CODEC,
    };
    if (wave_player_init(&audio_hw_cfg) != 0) {
        printf("[ERROR] wave_player_init failed\r\n");
        while (1) {}
    }
    if (wave_player_set_volume(85) != 0) {
        printf("[WARN] set default audio volume failed\r\n");
    }
    if (wave_player_set_record_volume(100) != 0) {
        printf("[WARN] set default record volume failed\r\n");
    }
    ...
    printf("audio_dac_adc ready: builtin auadc capture + builtin audac playback\r\n");
    vTaskStartScheduler();
