...
#define REF_ID           (0x563)
#define REF_ID_TYPE      (CANFD_ID_EXTENDED)
...
void canfd_isr(int irq, void *arg)
{
    ...
    if (intstatus & CANFD_INTSTS_TT_TC) {
        ...
        printf("tt tx complete\r\n");
        static uint32_t i = 1;
        if (i < TT_MESSAGE_COUNT) {
            tx_head.identifier = i * 0x111;
            tt_message_normal.slot = i;
            tt_message_normal.trig_time = i * 10000;
            bflb_canfd_tt_write_tb(canfd, &tx_head, tx_buffer, &tt_message_normal);
            i++;
        } else {
            i = 1;
        }
    ...
    if (intstatus & CANFD_INTSTS_RB_AVAILABLE) {
        ...
        if (rx_head.identifier == REF_ID && rx_head.id_type == REF_ID_TYPE) {
            tx_head.identifier = 0x123;
            tt_message_normal.slot = 0;
            tt_message_normal.trig_time = 5000;
            bflb_canfd_tt_write_tb(canfd, &tx_head, tx_buffer, &tt_message_normal);
        }
    }
}
...
int main(void)
{
    ...
    bflb_canfd_tt_set_reference_id(canfd, REF_ID, REF_ID_TYPE);
