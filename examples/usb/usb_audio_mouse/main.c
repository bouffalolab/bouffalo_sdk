...
#define USB_AUDIO_CONFIG_DESC_SIZ (unsigned long)(\
            9 +\
            9 +\
            9 +  \
            12 + \
...
int main(void)
{
	...
	usbd_hid_descriptor_register(hid_desc);
	usbd_hid_report_descriptor_register(hid_mouse_report_desc,HID_MOUSE_REPORT_DESC_SIZE);
    usb_fs = usb_dc_init();
    if (usb_fs)
    {
