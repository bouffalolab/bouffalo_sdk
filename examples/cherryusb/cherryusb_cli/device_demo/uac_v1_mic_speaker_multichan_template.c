...
#if USING_FEEDBACK == 0
    AUDIO_AS_DESCRIPTOR_INIT(0x01, 0x04, OUT_CHANNEL_NUM, AUDIO_SPEAKER_FRAME_SIZE_BYTE, AUDIO_SPEAKER_RESOLUTION_BIT, AUDIO_OUT_EP, 0x09, AUDIO_OUT_PACKET,
                             EP_INTERVAL, AUDIO_SAMPLE_FREQ_3B(AUDIO_SPEAKER_FREQ)),
#else
    AUDIO_AS_FEEDBACK_DESCRIPTOR_INIT(0x01, 0x04, OUT_CHANNEL_NUM, AUDIO_SPEAKER_FRAME_SIZE_BYTE, AUDIO_SPEAKER_RESOLUTION_BIT, AUDIO_OUT_EP, AUDIO_OUT_PACKET,
                                      EP_INTERVAL, AUDIO_OUT_FEEDBACK_EP, AUDIO_SAMPLE_FREQ_3B(AUDIO_SPEAKER_FREQ)),
#endif
    AUDIO_AS_DESCRIPTOR_INIT(0x02, 0x03, IN_CHANNEL_NUM, AUDIO_MIC_FRAME_SIZE_BYTE, AUDIO_MIC_RESOLUTION_BIT, AUDIO_IN_EP, 0x05, AUDIO_IN_PACKET,
...
static void usbd_event_handler(uint8_t busid, uint8_t event)
{
    switch (event) {
        ...
        case USBD_EVENT_RESUME:
            USB_LOG_INFO("UAC USBD_EVENT_RESUME\r\n");
            break;
        case USBD_EVENT_SUSPEND:
            USB_LOG_INFO("UAC USBD_EVENT_SUSPEND\r\n");
            break;
        case USBD_EVENT_CONFIGURED:
            USB_LOG_INFO("UAC USBD_EVENT_CONFIGURED\r\n");
...
#endif
        USB_LOG_INFO("UAC OPEN SPK\r\n");
    } else {
        tx_flag = 1;
        ep_tx_busy_flag = false;
        USB_LOG_INFO("UAC OPEN MIC\r\n");
    }
}
...
void usbd_audio_close(uint8_t busid, uint8_t intf)
{
    if (intf == 1) {
        rx_flag = 0;
        USB_LOG_INFO("UAC CLOSE SPK\r\n");
    } else {
        tx_flag = 0;
        ep_tx_busy_flag = false;
        USB_LOG_INFO("UAC CLOSE MIC\r\n");
    }
}
...
void uac_v1_test_polling(uint8_t busid)
{
    ...
    if (tx_flag) {
        for (int i = 0; i < AUDIO_IN_PACKET; i++) {
            ((uint8_t *)write_buffer)[i] = ((uint8_t *)sin_0db_l32_r16_2ch)[pcm_offset % 128];
            pcm_offset++;
        }
