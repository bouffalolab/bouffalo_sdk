...
extern void codec_bt_hfp_cb(uint16_t interval_halfslot,
...
static TaskHandle_t app_codec_handle;

static void demo_hfp_init(void)
{
    codec_hfp_init();
    /* codec mic */
    hfp_codec_cfg_t mic_config;
    mic_config.sample_rate = 8000;
    mic_config.bit_width = 16;
    hfp_codec_input_init(mic_config);

    /* codec speaker */
    hfp_codec_cfg_t speaker_config;
    speaker_config.sample_rate = 8000;
    speaker_config.bit_width = 16;
    hfp_codec_output_init(speaker_config);
}

static void codec_init_task(void *pvParameters)
{
    vTaskDelay(2000);
    demo_hfp_init();
    vTaskDelete(NULL);
...
int main(void)
{
    ...
    xTaskCreate(codec_init_task, (char *)"codec_init", 1024, NULL, 15, &app_codec_handle);
