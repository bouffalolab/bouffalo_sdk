...
/* Endpoint structures */
struct rpmsg_endpoint ep1_ept;
struct rpmsg_endpoint ep2_ept;
struct rpmsg_endpoint ep3_ept;

struct rpmsg_endpoint *ep1_ep = &ep1_ept;
struct rpmsg_endpoint *ep2_ep = &ep2_ept;
struct rpmsg_endpoint *ep3_ep = &ep3_ept;

/* Message reception functions */
static unsigned int receive_ep1_message(void)
{
    xSemaphoreTake(ep1_rx_sem, portMAX_DELAY);
    return ep1_data;
}
...
static void app_task(void *param)
{
    ...
    printf("RPMsg Service demo ended.\n");
    vTaskDelete(NULL);
}

static int rpmsg_master_app(void)
{
    /* Create semaphores */
    ep1_rx_sem = xSemaphoreCreateBinary();
    if (!ep1_rx_sem) {
        return -1;
    }
...
static int register_endpoint(void)
{
    ...
    ep1_ep_id = status;
    printf("Registered ep1 endpoint with ID: %d\n", ep1_ep_id);
    ...
    ep2_ep_id = status;
    printf("Registered ep2 endpoint with ID: %d\n", ep2_ep_id);
    ...
    ep3_ep_id = status;
    printf("Registered ep3 endpoint with ID: %d\n", ep3_ep_id);

    return 0;
}

/* Initialize the system */
int master_rpmsg_init(void)
{
    int ret;
